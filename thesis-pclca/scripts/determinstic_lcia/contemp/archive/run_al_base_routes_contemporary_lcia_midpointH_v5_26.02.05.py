# -*- coding: utf-8 -*-
"""
run_al_base_routes_contemporary_lcia_midpointH_v4_26.02.05.py
...
- c3c4:         route chain impacts only (C3/C4 wrapper; no explicit Stage D node)
- staged_total: Stage D credit only (if route has explicit Stage D node)
- joint:        c3c4 + Stage D credit together (if route has explicit Stage D node)
- net:          NET wrapper (if available), intended to be equivalent to joint for explicit Stage-D routes

Notes on recycling credit modes:
- If BW_RECYCLE_CREDIT_MODE=rewire_embedded, recycling credits are embedded in the refiner, so Stage D node may not exist.
  In that case, 'stageD' is treated as <none> for recycling and you still get 'c3c4' and 'net'.
- If BW_RECYCLE_CREDIT_MODE=external_stageD, recycling Stage D node exists and net should match joint (QA net-joint check).
...
# stageD_required:
#   - reuse: True (builder always creates reuse Stage D)
#   - recycling_postcons: False (depends on BW_RECYCLE_CREDIT_MODE)
#   - landfill: False (no Stage D)
ROUTES = {
    "reuse": {
        ...
        "stageD_component_codes": {
            ...
            "stageD_extrusion_only": ["AL_SD_credit_reuse_CA_extrusion_only"],  # build script uses CA
        },
    ...
    "recycling_postcons": {
        ...
        # Only exists if BW_RECYCLE_CREDIT_MODE=external_stageD
...
def _as_code_list(x: Union[None, str, List[str]]) -> List[str]:
    if x is None:
        return []
    if isinstance(x, str):
        return [x]
...
def pick_stageD(
    ...
    *,
    ...
    # Optional fallback search if provided
    if fallback_search:
        hits = db.search(fallback_search, limit=400) or []
        if hits:
            hint = [(t or "").lower() for t in (score_hint_terms or [])]
            ...
            def score(a) -> int:
                ...
                for t in hint:
                    if t and (t in nm or t in cd):
                        sc += 20
                # bias toward obvious credit nodes
...
def list_recipe_midpointH_methods(logger: logging.Logger) -> List[Tuple[str, str, str]]:
    ...
    for m in list(bw.methods):
        ...
        if m[0] != "ReCiPe 2016 v1.03, midpoint (H)":
            continue
        # FIX: "no LT" typically appears in the joined label, not in m[0]/m[1]/m[2] individually
...
def _is_nonsquare_exception(e: Exception) -> bool:
    ...
    try:
        ...
        pass
    # Fallback to message match (older behavior)
    msg = str(e)
...
def main():
    ...
    for route_name in ROUTE_SELECTION:
        ...
        # Stage D: robust (codes list + optional fallback); required only for reuse
        stageD = pick_stageD(
            fg_db,
            cfg.get("stageD_codes") or cfg.get("stageD_code"),
            logger,
            label=f"{route_name} (Stage D credit)",
            required=bool(cfg.get("stageD_required", False)),
