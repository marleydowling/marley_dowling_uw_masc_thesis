# -*- coding: utf-8 -*-
"""
run_al_base_routes_contemporary_lcia_midpointH_v6_26.02.10.py

Contemporary Aluminium BASE ROUTES LCIA run (ReCiPe 2016 Midpoint - Hierarchist, DEFAULT LT).

Key change vs v4:
- recycling_postcons Stage D becomes REQUIRED iff BW_RECYCLE_CREDIT_MODE=external_stageD
  (mirrors prospective external-stageD runner logic)

Nonsquare handling:
- Same as v4 (LeastSquares fallback if available).
"""
...
# Read build-mode to decide required Stage D for recycling
RECYCLE_CREDIT_MODE = os.environ.get("BW_RECYCLE_CREDIT_MODE", "rewire_embedded").strip().lower()
REQUIRE_RECYCLE_STAGE_D_IF_EXTERNAL = True  # hard rule to match prospect runner behavior
...
ROUTES = {
    ...
    "recycling_postcons": {
        ...
        "stageD_required": False,  # will be overridden dynamically below
        "stageD_component_codes": {},
...
def setup_logger(root: Path) -> logging.Logger:
    ...
    log_path = logs_dir / f"run_al_base_routes_contemp_recipe2016_midpointH_v5_{ts}.log"

    logger = logging.getLogger("run_al_base_routes_contemp_midpointH_v5")
...
def main():
    ...
    # Dynamic requirement: recycling Stage D only required in external_stageD mode
    if REQUIRE_RECYCLE_STAGE_D_IF_EXTERNAL and RECYCLE_CREDIT_MODE == "external_stageD":
        ROUTES["recycling_postcons"]["stageD_required"] = True
        logger.info("[cfg] Recycling Stage D REQUIRED (external_stageD mode).")
    else:
        ROUTES["recycling_postcons"]["stageD_required"] = False
        logger.info("[cfg] Recycling Stage D OPTIONAL (embedded-credit mode).")
