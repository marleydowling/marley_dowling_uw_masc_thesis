# -*- coding: utf-8 -*-
"""
run_al_base_routes_contemporary_lcia_midpointH_NET_v17_26.02.15.py

Contemporary Aluminium BASE ROUTES LCIA run (ReCiPe 2016 Midpoint - H, default LT).

Key change vs v16:
- All split-case demands are *linked* using the NET wrapper's technosphere coefficients.
  This guarantees that:
    - c3c4-only uses the c3c4 amount implied by the NET wrapper per FU
    - staged_total uses the stageD amount implied by the NET wrapper per FU
    - joint uses both consistently
  rather than assuming they are both exactly FU.
...
Outputs:
- Long + wide CSV across all ReCiPe 2016 Midpoint (H) default LT categories
- TopN contributors for PRIMARY method per case

Nonsquare handling:
- Falls back to LeastSquaresLCA when needed.
"""
...
# =============================================================================
# Env / mode normalization (cleaned)
# =============================================================================

def normalize_credit_mode(raw: str) -> str:
    s = (raw or "").strip().lower().replace("-", "_").replace(" ", "_")
    if s in {"external_staged", "external_stage", "external_stage_d", "external_staged_d"}:
        return "external_stageD"
    if s in {"external_stagedd"}:
        return "external_stageD"
    if s in {"external_stagedd"}:
        return "external_stageD"
    if s in {"rewire_embedded", "rewire", "embedded"}:
        return "rewire_embedded"
    return s if s else "rewire_embedded"
...
def list_recipe_midpointH_methods(exclude_no_lt: bool, logger: logging.Logger) -> List[Tuple[str, str, str]]:
    ...
    logger.info(f"[method] Total ReCiPe 2016 Midpoint (H) default LT methods found: {len(methods)}")
    if not methods:
    ...
    def score(a) -> int:
        ...
        for t in hint:
            if t and (t in nm or t in cd):
                sc += 25
        if "no_credit" in nm or "no credit" in nm or "no_credit" in cd:
            sc -= 200
...
# =============================================================================
# Linked-demand helpers (via NET wrapper coefficients)
# =============================================================================
...
def net_child_coeff(net_act, child_act) -> Optional[float]:
    """
    Return the technosphere coefficient in net_act that references child_act.
    """
    target_key = child_act.key
    ...
    try:
        ...
        if _is_nonsquare_exception(e):
            ...
            if hasattr(bc, "LeastSquaresLCA"):
                logger.warning("[lci] Falling back to LeastSquaresLCA (provisional results).")
                lca = bc.LeastSquaresLCA(demand, method)
                lca.lci()
                return lca
            raise RuntimeError("Technosphere is nonsquare and LeastSquaresLCA unavailable.")
        raise
...
# =============================================================================
# Route config (aligned to build outputs)
# =============================================================================
...
def run_routes(
    *,
    ...
    logger.info(f"[cfg] include_net_wrapper={include_net_wrapper}")
    logger.info("=" * 90)
    ...
    for route in routes:
        cfg = ROUTES[route]
        include_stageD, _ = stageD_policy(route, credit_mode)

        logger.info("-" * 90)
        logger.info(f"[route] {route} (include_stageD={include_stageD})")
        ...
        # LINKED scaling from NET wrapper
        c3_coeff = net_child_coeff(net, c3c4)
        if c3_coeff is None or c3_coeff <= 0:
            logger.warning(f"[link][WARN] NET does not reference c3c4 (or coeff<=0). Defaulting c3_coeff=1.0")
            c3_coeff = 1.0

        sd_coeff = None
        if stageD is not None:
            sd_coeff = net_child_coeff(net, stageD)
            if sd_coeff is None or sd_coeff <= 0:
                logger.warning(f"[link][WARN] NET does not reference stageD (or coeff<=0). Defaulting sd_coeff=1.0")
                sd_coeff = 1.0

        logger.info(f"[link] route={route} coeff(net→c3c4)={c3_coeff:.12g}  coeff(net→stageD)={(sd_coeff if sd_coeff is not None else '<n/a>')}")

        # Cases (split; but demands are now linked)
        cases: Dict[str, Dict[Any, float]] = {}
        cases["c3c4"] = {c3c4: fu_al_kg * float(c3_coeff)}

        if stageD is not None:
            cases["staged_total"] = {stageD: fu_al_kg * float(sd_coeff)}
            cases["joint"] = {
                c3c4: fu_al_kg * float(c3_coeff),
                stageD: fu_al_kg * float(sd_coeff),
            }
...
def main():
    ...
    logger = setup_logger(DEFAULT_ROOT, "run_al_base_routes_contemp_recipe2016_midpointH_NET_v17")
