# -*- coding: utf-8 -*-
"""
Deterministic LCA runner (CONTEMPORARY) for hydrolysis + Stage D activities.
Exports ALL ReCiPe 2016 midpoint (H) categories found in the project to CSV.

Outputs to:
  C:\brightway_workspace\results\0_contemp\hydrolysis
"""
...
OUT_DIR = r"C:\brightway_workspace\results\0_contemp\hydrolysis"

# Activity code prefixes to include (will pick all matches)
CODE_PREFIXES = [
...
# Method selection: all ReCiPe 2016 midpoint (H) categories (adjust if desired)
...
def main():
    ...
    out_long = os.path.join(OUT_DIR, f"det_lca_contemp_hydrolysis_{ts}_long.csv")
    out_wide = os.path.join(OUT_DIR, f"det_lca_contemp_hydrolysis_{ts}_wide.csv")

    # LONG format
    rows_long = []

    # Compute per-activity scores for all methods (LCI once per activity)
    ...
    # Add TOTAL row by summing component scores per method
    total_code = "TOTAL_C3C4_plus_StageD"
    for m in methods:
        total = 0.0
        for a in acts:
            total += scores[a.get("code")][m]
        rows_long.append({
