# -*- coding: utf-8 -*-
"""
Deterministic LCA runner for contemporary aluminium hydrolysis routes.

Modules:
- C3–C4 only (hydrolysis route with Stage D credits removed)
- Stage D only (H2 + Al(OH)3 avoided-burden, per 1 kg scrap at C3 gate)
- Joint (C3–C4 + Stage D as actually modelled in the hydro route)

Conceptual setup
----------------
In the contemporary model:

- The hydrolysis treatment activity `al_hydrolysis_treatment_CA` already
  includes Stage D credits indirectly via *negative* inputs to the H2 and
  Al(OH)3 Stage D proxy activities (per kg of co-product).

- The Stage D proxy activities themselves represent avoided production of:
    * Hydrogen, marginal supply, SMR proxy, AB, contemporary
    * Aluminium hydroxide (Al(OH)3) offset proxy

- We reconstruct "modules" per 1 kg aluminium scrap at the C3 gate using
  functional-unit composition with the co-product yields:

  Let:
    H2_USABLE_KG_PER_KG_SCRAP   = 0.085977  kg H2 / kg scrap
    ALOH3_KG_PER_KG_SCRAP       = 2.334377 kg Al(OH)3 / kg scrap

  Then:

    JOINT (C3–C4 + D as modelled):
      FU_joint   = { C3C4: 1.0 }

    Stage D only (credit, typically negative LCIA):
      FU_stageD  = {
          D_H2:    +H2_USABLE_KG_PER_KG_SCRAP,
          D_ALOH3: +ALOH3_KG_PER_KG_SCRAP,
      }

      -> Avoided production of H2 + Al(OH)3 per kg scrap.

    C3–C4 only (burdens-only module, with Stage D removed):
      FU_c3c4    = {
          C3C4:    1.0,
          D_H2:   -H2_USABLE_KG_PER_KG_SCRAP,
          D_ALOH3:-ALOH3_KG_PER_KG_SCRAP,
      ...
      -> Joint - StageD in functional-unit space.

By construction, LCIA scores satisfy:
    JOINT ≈ C3–C4 + Stage D

Outputs
-------
- LCIA score for each module + joint (GWP1000, kg CO2-eq and tCO2e)
- Top contributing technosphere activities under GWP1000 (absolute contribution)
- Top biosphere flows
...
from bw2data.utils import get_node  # resolves int node IDs or (db, code)
...
# (must match the hydrolysis foreground build + Stage D setup)
...
def main():
    ...
    # Functional units per 1 kg scrap at C3 gate
    # Stage D = avoided co-product production (credit, usually negative)
    fu_stageD = {
        d_h2:    H2_USABLE_KG_PER_KG_SCRAP,
        d_aloh3: ALOH3_KG_PER_KG_SCRAP,
    }

    # C3–C4-only = Joint (hydro route) minus Stage D credit
    fu_c3c4 = {
        c3c4:    1.0,
        d_h2:   -H2_USABLE_KG_PER_KG_SCRAP,
        d_aloh3:-ALOH3_KG_PER_KG_SCRAP,
    ...
    # Joint = hydrolysis route as actually modelled (C3–C4 + Stage D inside)
    fu_joint = {c3c4: 1.0}

    logger.info(
        "[fu] C3–C4:   1 * C3C4 - %.6f * H2_proxy - %.6f * Al(OH)3_proxy",
        H2_USABLE_KG_PER_KG_SCRAP,
        ALOH3_KG_PER_KG_SCRAP,
    )
    logger.info(
    ...
    # GWP-only run with detailed contributors
    ...
    logger.info(
        "[check] joint - (c3c4 + stageD) = %.12f kg CO2-eq (should be ~0)",
        diff,
    )
    ...
    # All impact categories (ReCiPe 2016 midpoint (E) no LT) for CSV
    ...
    if all_methods:
        ...
        logger.warning(
            "[methods] No methods found for family root '%s'; skipping all-impacts CSV.",
            METHOD_FAMILY_ROOT,
        )
    ...
    if WRITE_REPORT:
        ...
        payload = {
            ...
            "notes": {
                "sign_convention": (
                    "Stage D is modelled as avoided co-product production (credit). "
                    "Here, Stage D-only is the avoided burden per 1 kg scrap at the C3 gate. "
                    "C3–C4-only subtracts Stage D from the hydrolysis route, and JOINT is "
                    "the hydro route as modelled. Scores satisfy JOINT ≈ C3–C4 + StageD."
                ),
                "contributors_basis": (
                    "Column-sum / row-sum of characterized inventory; ranked by absolute value."
                ),
                "bw2calc_compat": (
                    "Handles activity_dict/biosphere_dict keys as either node IDs (int) "
                    "or (db, code) tuples via bw2data.utils.get_node."
                ),
            },
        }
