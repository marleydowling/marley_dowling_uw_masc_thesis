...
ROUTES = {
    "reuse": {
        ...
        "stageD_component_codes": {
            ...
            # FIX: build script uses CA for extrusion-only
    ...
    "recycling_postcons": {
        ...
        # OPTIONAL: only exists if BW_RECYCLE_CREDIT_MODE=external_stageD
...
def pick_stageD_optional(db, code: Optional[str], logger: logging.Logger, label: str):
    ...
    if act is None:
        # OPTIONAL behavior: return None rather than hard fail
        logger.warning(f"[pick][WARN] {label}: code not found '{code}' -> treating as <none> (likely built in rewire_embedded mode).")
        return None
    logger.info(f"[pick] {label}: {act.key} loc={act.get('location')} name='{act.get('name')}'")
    return act
