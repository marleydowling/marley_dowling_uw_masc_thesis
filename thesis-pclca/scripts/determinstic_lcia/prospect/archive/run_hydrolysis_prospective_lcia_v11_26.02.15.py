# -*- coding: utf-8 -*-
"""
run_hydrolysis_prospective_lcia_v14_26.02.15.py

Prospective hydrolysis LCIA run (ReCiPe 2016 Midpoint - Hierarchist, DEFAULT LT)
Aligned to v14 PROSPECTIVE build (GATE BASIS).

Build alignment (v14):
- C3–C4 hydrolysis chain activity (GATE BASIS; per 1 kg gate scrap):
    al_hydrolysis_treatment_CA_GATE_BASIS__{SCEN_ID}
- Stage D is a SINGLE credit-only activity with TWO negative technosphere exchanges (GATE BASIS):
    al_hydrolysis_stageD_offsets_CA_GATE_BASIS__{SCEN_ID}
- Gate A (scrap-at-gate; routing removed; no implicit prepared-scrap market routing):
    al_scrap_postconsumer_CA_gate__{SCEN_ID}
- Prep/shredding (per 1 kg prepared output):
    al_scrap_shredding_for_hydrolysis_CA__{SCEN_ID}

Functional unit basis (IMPORTANT):
- In v14, C3C4 + StageD + GateA are defined on a "kg gate scrap" basis.
- Prep is defined on a "kg prepared scrap output" basis.
This runner still computes the same 5 cases as before; just be mindful that "prep_only"
is not on the same basis as the other cases unless you explicitly scale it.

Computes 5 cases per scenario:
    * gateA_only   : Gate A only
...
# Demand units into each requested activity.
# In v14 build: C3C4 + StageD + GateA are on kg gate scrap basis; Prep is per kg prepared output basis.
FU_UNITS = 3.67
...
def setup_logger(root: Path) -> logging.Logger:
    ...
    log_path = logs_dir / f"run_hydrolysis_prospect_recipe2016_midpointH_v14_{ts}.log"

    logger = logging.getLogger("run_hydrolysis_prospect_midpointH_all_v14")
...
# =============================================================================
# PICKERS (v14-aligned, with backward-compatible fallbacks)
# =============================================================================
...
def pick_c3c4_for_scenario(fg_db, tag: str, logger: logging.Logger):
    # v14: al_hydrolysis_treatment_CA_GATE_BASIS__{SCEN_ID}
    # keep older variants as fallbacks
    codes = [
...
def pick_stageD_for_scenario(fg_db, tag: str, logger: logging.Logger):
    # v14: al_hydrolysis_stageD_offsets_CA_GATE_BASIS__{SCEN_ID}
    codes = [
...
def pick_gateA_for_scenario(fg_db, tag: str, logger: logging.Logger):
    # v14: al_scrap_postconsumer_CA_gate__{SCEN_ID}
    codes = [
        f"al_scrap_postconsumer_CA_gate__{tag}",
        f"al_scrap_postconsumer_CA_gate_{tag}",

        # older runner fallback (if you still have it in DB):
...
def quick_mixing_check(
    ...
    max_exchanges: int = 250
) -> None:
    """
    NOTE: For top-level FG wrapper nodes, it's normal that the *direct* technosphere inputs
    are other FG nodes (so you may not "see" the BG db here). The post-LCI supply_db_counts()
    check is the stronger signal.
    """
    try:
    ...
    if expected_bg_db not in db_counts:
        _p(
            logger,
            f"[mixcheck][INFO] Expected BG DB '{expected_bg_db}' not seen in *direct* inputs "
            f"(often normal for FG wrapper nodes).",
            level="warning",
        )
...
def run_scenario(
    ...
    _p(logger, f"[FU] Demand basis = {FU_UNITS} unit(s) into each requested activity.")
    _p(logger, f"[FU] v14 note: C3C4/StageD/GateA are kg-gate basis; Prep is kg-prepared basis.")
...
def main():
    ...
    _p(logger, "[done] Prospective Midpoint (H) run complete (3 scenarios; 5 cases; v14-aligned; all midpoint categories).")
