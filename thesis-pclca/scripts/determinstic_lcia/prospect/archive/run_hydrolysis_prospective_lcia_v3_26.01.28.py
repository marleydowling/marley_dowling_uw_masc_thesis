# run_hydrolysis_c3c4_stageD_ipcc_summary_prospect_v3_26.01.28.py
...
C3C4_CODE_HINTS = ["al_hydrolysis_treatment"]
C3C4_NAME_MUST_CONTAIN = ["hydrolysis", "treatment"]
C3C4_LOC_PREFER = ["CA", "CA-QC", "CA-ON", "CA-BC", "CA-AB", "RNA", "RoW", "GLO"]

STAGED_CODE_HINTS = ["stageD", "hydrolysis"]
STAGED_NAME_MUST_CONTAIN = ["stage d", "credit"]
...
def setup_logger(root: Path) -> logging.Logger:
    ...
    log_path = logs_dir / f"run_hydrolysis_ipcc_summary_prospect_{ts}.txt"

    logger = logging.getLogger("run_hydrolysis_ipcc_summary_prospect")
