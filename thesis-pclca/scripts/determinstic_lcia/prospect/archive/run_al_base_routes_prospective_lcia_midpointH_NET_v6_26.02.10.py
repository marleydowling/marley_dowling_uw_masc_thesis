# -*- coding: utf-8 -*-
"""
run_al_base_routes_prospective_lcia_midpointH_NET_v6_26.02.10.py
...
def assert_external_stageD_architecture(net_act, c3c4_act, stageD_act, logger, tag, route) -> None:
    ...
    if not net_wrapper_references(net_act, c3c4_act):
        missing.append("c3c4")
    if stageD_act is not None and (not net_wrapper_references(net_act, stageD_act)):
    ...
    _p(logger, f"[qa] {tag} route={route} external Stage D architecture OK (NET references c3c4 + stageD).")


# =============================================================================
# LCA helpers (nonsquare-safe)
# =============================================================================
...
def build_lca(demand: Dict[Any, float], method: Tuple[str, str, str], logger: logging.Logger):
    """
    Returns:
      - lca object if solvable
      - None if nonsquare and skipping is enabled and LS not available
    """
    try:
...
def run_one_scenario(
    ...
    # Resolve activities + build demand cases
    for route in ROUTE_SELECTION:
        # --- resolve c3c4 wrapper ---
        ...
        # --- resolve stageD credit ---
        if route in ("reuse", "recycling_postcons"):
            ...
            stageD = None
            _p(logger, f"[pick] {tag} :: {route} :: stageD = <none>")
        ...
        # Enforce architecture for external-stageD routes
        if route in ("reuse", "recycling_postcons"):
    ...
    for route, cases in route_demands.items():
        ...
        # QA: net should match joint for external-stageD routes
        if ("net" in primary_scores) and ("joint" in primary_scores) and (route in ("reuse", "recycling_postcons")):
...
def main():
    ...
    # Optional datapackage check (non-fatal)
    try:
