"""
run_hydrolysis_prospective_lcia_midpointH_all_v7_2026.02.01.py
...
def list_recipe_midpointH_methods(logger: logging.Logger) -> List[Tuple[str, str, str]]:
    """
    Collect ALL ReCiPe 2016 v1.03 midpoint (H) methods (default LT).
    Excludes "no LT" variants by default to align with standard building practice.
    """
    methods = []
...
def run_scenario(
    ...
    for case_name, demand in demands.items():
        ...
        _p(logger, f"[lcia] start {tag} {case_name} method={primary_method}")
        lca.lcia()
