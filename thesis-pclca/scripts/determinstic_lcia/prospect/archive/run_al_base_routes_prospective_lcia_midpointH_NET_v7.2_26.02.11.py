...
"""
Run LCIA for prospective scenario-suffixed base route wrappers.

Reports: c3c4, stageD_only, net, joint, QA net_minus_joint (primary).
"""
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("run_al_base_routes_prospect")
...
def extract_c3c4_and_stageD(net_act: bd.backends.base.Activity) -> Tuple[bd.backends.base.Activity, Optional[bd.backends.base.Activity]]:
    ...
    for exc in net_act.exchanges():
        ...
        if code.startswith("AL_SD_"):
            ...
            c3 = inp
    if c3 is None:
        raise RuntimeError(f"Could not extract c3c4 input from NET: {net_act.key}")
    return c3, sd
...
def main() -> None:
    ...
    log_path = logs_dir / f"run_al_base_routes_prospect_NET_v10_{now_tag()}.log"
    ...
    wide_path = out_dir / f"al_base_routes_prospect_NET_v10_wide_{now_tag()}.csv"
    long_path = out_dir / f"al_base_routes_prospect_NET_v10_long_{now_tag()}.csv"
    ...
    for sid in args.scenario_ids:
        ...
        net_codes = {
            "reuse": f"AL_RW_reuse_NET_CA__{sid}",
            "recycling_postcons": f"AL_RW_recycling_postcons_NET_CA__{sid}",
            "landfill": f"AL_RW_landfill_NET_CA__{sid}",
        }

        for route, net_code in net_codes.items():
            ...
            if stageD_required and sd_act is None:
                raise RuntimeError(f"[arch] Stage D required but missing for {sid} route='{route}' NET={net_act.key}")
            ...
            for case_name, scores in [
                ...
                wide = {"scenario": sid, "route": route, "case": case_name}
                for m in methods:
    ...
    long_fields = ["scenario", "route", "case", "method_0", "method_1", "method_2", "score"]
