...
Runner aligned to build_al_base_routes_prospect_NET_v5_26.02.04.py
...
def top_process_contributions(lca: bc.LCA, limit: int = 20) -> pd.DataFrame:
    """
    Compute absolute characterized contributions by activity (signed),
    then return top |contribution| processes.
    """
...
# =============================================================================
# NONSQUARE DIAGNOSTICS + LCA handling
# =============================================================================
...
def run_scenario(tag, bg_db_name, fg_db, methods, primary_method, logger, out_root):
    ...
    for (route, case_name), demand in demands.items():
        ...
        # Build LCA for PRIMARY (with nonsquare handling)
        ...
        for m in methods:
            ...
            try:
                ...
                # Rebuild under this method (and handle nonsquare)
