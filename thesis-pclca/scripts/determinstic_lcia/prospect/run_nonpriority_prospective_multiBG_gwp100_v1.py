# -*- coding: utf-8 -*-
"""
run_nonpriority_prospective_multiBG_gwp100_v1.py

Runs GWP100 LCIA for non-priority material wrappers across prospective
scenario foreground DBs.

It is robust to multiple possible scenario FG DB naming conventions,
including the exact naming implied by your build script:
    mtcw_foreground_prospectiveSSP1VLLO_2050
    mtcw_foreground_prospectiveSSP2M_2050
    mtcw_foreground_prospectiveSSP5H_2050

Outputs:
- detailed per-material per-scenario results
- total non-priority material summary by scenario
...
SCENARIOS = {
    "SSP1VLLO_2050": [
        "mtcw_foreground_prospectiveSSP1VLLO_2050",     # exact from your build script
        "mtcw_foreground_prospective__SSP1VLLO_2050",
        "mtcw_foreground_prospective_SSP1VLLO_2050",
    ],
    "SSP2M_2050": [
        "mtcw_foreground_prospectiveSSP2M_2050",
        "mtcw_foreground_prospective__SSP2M_2050",
        "mtcw_foreground_prospective_SSP2M_2050",
    ],
    "SSP5H_2050": [
        "mtcw_foreground_prospectiveSSP5H_2050",
        "mtcw_foreground_prospective__SSP5H_2050",
        "mtcw_foreground_prospective_SSP5H_2050",
    ],
}
...
def resolve_fg_db(candidates: List[str]) -> Optional[str]:
...
def main() -> None:
    ...
    detailed_csv = OUT_DIR / f"nonpriority_prospective_detailed_{ts}.csv"
    totals_csv = OUT_DIR / f"nonpriority_prospective_totals_{ts}.csv"
    summary_txt = OUT_DIR / f"nonpriority_prospective_summary_{ts}.txt"
    ...
    for scenario, fg_candidates in SCENARIOS.items():
        fg_db = resolve_fg_db(fg_candidates)
            ...
            total_rows.append(
                {
                    "scenario": scenario,
                    "fg_db": None,
                    "status": "missing_fg_db",
                    "c3c4_total_kgco2e_per_m2": None,
                    "stageD_total_kgco2e_per_m2": None,
                    "net_total_kgco2e_per_m2": None,
                }
            )
            continue
        ...
        for item in NONPRIORITY_BASIS:
            ...
            try:
                c3_act = get_activity(fg_db, c3_code)
            ...
            try:
                d_act = get_activity(fg_db, d_code)
        ...
        total_rows.append(
            {
                ...
                "fg_db": fg_db,
                "status": "ok",
                "c3c4_total_kgco2e_per_m2": totals["c3c4_total_kgco2e_per_m2"],
                "stageD_total_kgco2e_per_m2": totals["stageD_total_kgco2e_per_m2"],
                "net_total_kgco2e_per_m2": totals["net_total_kgco2e_per_m2"],
            }
        )

    write_csv(detailed_csv, rows)
    write_csv(totals_csv, total_rows)

    with summary_txt.open("w", encoding="utf-8") as f:
        f.write("Appendix B — Non-priority material GWP100 summary (Prospective)\n")
        f.write("===============================================================\n\n")
        f.write(f"Project: {PROJECT_NAME}\n")
        f.write(f"Method: {PRIMARY_METHOD_EXACT}\n\n")

        for total in total_rows:
            f.write(f"{total['scenario']}\n")
            f.write("-" * len(str(total["scenario"])) + "\n")
            f.write(f"FG DB: {total['fg_db']}\n")
            f.write(f"Status: {total['status']}\n")
            if total["status"] == "ok":
                f.write(f"C3–C4 total: {total['c3c4_total_kgco2e_per_m2']:+.8f}\n")
                f.write(f"Stage D total: {total['stageD_total_kgco2e_per_m2']:+.8f}\n")
                f.write(f"Net total: {total['net_total_kgco2e_per_m2']:+.8f}\n")
            f.write("\n")

        f.write("Excluded materials:\n")
        f.write("-------------------\n")
