"""
run_msfsc_prospective_lcia_midpointH_v3_26.02.16.py
...
def pick_route_total(fg_db, tag: str, logger: logging.Logger) -> Optional[Any]:
    ...
    for c in codes:
        ...
        if act is not None:
            logger.info(
                f"[pick] Route TOTAL [{tag}]: {act.key} loc={act.get('location')} "
                f"code={act.get('code') or act.key[1]} name='{act.get('name')}'"
            )
            return act
...
def pick_stageD_wrapper(fg_db, tag: str, logger: logging.Logger) -> Optional[Any]:
    ...
    for c in codes:
        ...
        if act is not None:
            logger.info(
                f"[pick] StageD wrapper [{tag}]: {act.key} loc={act.get('location')} "
                f"code={act.get('code') or act.key[1]} name='{act.get('name')}'"
            )
            return act
...
def infer_gateA_per_route(routeC, degrease, gateA, logger: logging.Logger) -> float:
    ...
    if (r_to_deg is not None and r_to_deg > 0) and (d_to_gate is not None and d_to_gate > 0):
        ...
        logger.info(
            f"[fu] gateA_per_route (via degrease) = {via:.12g} = "
            f"(route->degrease {r_to_deg:.12g}) * (degrease->gateA {d_to_gate:.12g})"
        )
        return via
...
def run_scenario(
    ...
    # -------------------------------------------------------------------------
    # FIX: define gateA_per_deg for meta output (previous script referenced
    # gateA_per_deg without defining it, causing NameError).
    # Functionality is otherwise unchanged.
    # -------------------------------------------------------------------------
    gateA_per_deg: Optional[float] = None
    try:
        gateA_per_deg = infer_gateA_per_degrease(deg, gateA, logger)
    except Exception as e:
        logger.warning(
            f"[fu][WARN] Could not infer gateA_per_degrease (will write null in meta): {type(e).__name__}: {e}"
        )
        gateA_per_deg = None
