# -*- coding: utf-8 -*-
"""
run_hydrolysis_prospect_lcia_midpointH_uncertainty_joint_v2_2026.02.26.py
...
- Optional NET wrapper: al_hydrolysis_route_total_NET_GATE_BASIS__{SCEN_ID}

Cases
-----
- c3c4         : demand on hydrolysis C3C4 node
- staged_total : demand on StageD offsets node (combined credits)
- joint        : c3c4 + staged_total
- net_wrapper  : optional direct demand on NET wrapper (diagnostic)

Foreground levers (Step-6 style)
--------------------------------
Sampled each iteration (PERT/lognormal) and used to overwrite technosphere coefficients:

- Y_prep      : prep yield (gate scrap -> prepared feed)
- f_Al, X_Al   : Al fraction + conversion
- L            : liquor ratio (L/kg Al-feed)
- f_makeup     : makeup fraction of working liquor
- R_PSA        : H2 usable fraction after PSA
- E_aux,E_therm: kWh/kg prepared (aux + thermal)

Derived per kg gate scrap treated:
- prep requires scrap input = 1 / Y_prep
- hydrolysis requires prepared feed = Y_prep
- electrolyte makeup, purge, stoich water, PSA service, electricity
- StageD credits: -H2_usable and -Al(OH)3

Optional: vary electrolyte composition by C_NaOH
------------------------------------------------
If --vary-c-naoh, an additional PERT draw varies the NaOH solution recipe inside the electrolyte activity
(naoh_proxy + water inputs per kg electrolyte output).

Robustness note (important)
---------------------------
Because we overwrite technosphere_matrix after next(lca), we force a refactorization
(decompose_technosphere / factorize / solver reset) before solving LCIs.
...
Usage
-----
python run_hydrolysis_prospect_lcia_midpointH_uncertainty_joint_v2_2026.02.26.py ^
  --scenario-ids SSP1VLLO_2050 SSP2M_2050 SSP5H_2050 ^
  --iterations 1000 --seed 42 --save-samples --fg-couple-across-scenarios 1

python run_hydrolysis_prospect_lcia_midpointH_uncertainty_joint_v2_2026.02.26.py ^
  --scenario-ids SSP2M_2050 --iterations 1500 --also-deterministic --write-qa-csv --include-net-wrapper
...
def set_csr_value(mat, row: int, col: int, value: float) -> None:
    """
    Update an existing CSR entry in-place (preferred).
...
def force_refactorization(lca: bc.LCA, logger: logging.Logger) -> None:
    """
    After manual technosphere_matrix overwrites, force the LCA to refactorize.
    Tries common BW2Calc APIs; falls back to solver invalidation.
    """
    ...
    # Fallback: invalidate cached solver-ish attributes
...
def run_monte_carlo_joint(
    ...
    hooks: Dict[str, Dict[str, Any]],  # sid -> indices + rho + optional recipe indices
    methods: List[Tuple[str, str, str]],
    primary_method: Tuple[str, str, str],
    iterations: int,
    seed: Optional[int],
    ...
    # Cache characterization matrices (deterministic; stable)
    ...
    for it in range(1, iterations + 1):
        ...
        # FG draws: coupled or per scenario
        ...
        # Apply FG overwrites (per scenario)
        for sid, hook in hooks.items():
            ...
            prepared_mass = Y                         # kg prepared per kg gate scrap treated
            al_feed = prepared_mass * fAl            # kg Al in feed
            reacted_al = al_feed * X                 # kg Al reacted

            # electrolyte makeup (kg solution) per kg gate
            ...
            set_csr_value(mc_lca.technosphere_matrix, hook["row_water_hyd"], hook["col_hyd"], stoich_h2o)

            # Stage D credits (combined wrapper)
        ...
        # Score each demand case using cached characterization matrices
...
def main():
    ...
    logger = setup_logger("run_hydrolysis_prospect_uncertainty_joint_midpointH_v2")
    ...
    # Resolve acts + build deterministic demands (objects) + MC demands (ids)
    ...
    for sid in scenario_ids:
        ...
        for exc in hyd.exchanges():
            ...
            if unit in ("kilowatt hour", "kwh") or rp.startswith("electricity") or "market for electricity" in nm:
        ...
        # Detect stoich/makeup water provider used by hydrolysis (exclude electrolyte + wastewater)
        ...
        if water_hyd_provider is None:
            raise RuntimeError(f"{sid}: Could not detect water provider on hydrolysis node for stoich water.")

        # Detect water provider used by electrolyte recipe (the non-naoh input)
        ...
        # QA: StageD should have exactly 2 negative technosphere exchanges (H2 + AlOH3)
        qa_stageD_offsets_has_two_neg(stageD, logger, strict=bool(args.strict_stageD))

        # QA: embedded credits in FG chain rooted at hydrolysis node
        ...
        ids_by_sid[sid] = {
            ...
            "water_hyd": int(water_hyd_provider.id),
        }
    ...
    # Build a union-demand LCA ONCE to obtain activity_dict indices + infer rho
    union_demand: Dict[int, float] = {}
    ...
    for sid, ids in ids_by_sid.items():
        ...
        row_water_hyd = lca0.activity_dict[ids["water_hyd"]]
        ...
        # optional electrolyte recipe overwrite indices
        ...
        # infer rho from central electrolyte->hyd coefficient
    ...
    logger.info("[done] Hydrolysis JOINT uncertainty LCIA run complete (v2).")
