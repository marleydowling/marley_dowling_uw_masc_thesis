# -*- coding: utf-8 -*-
"""
run_hydrolysis_prospect_lcia_midpointH_joint_v1_2026.02.27.py

Monte Carlo LCIA runner for PROSPECTIVE Hydrolysis (2050 SSP backgrounds) in the JOINT project/FG DB.

Aligned to:
  build_hydrolysis_prospect_joint_params_v4_2026.02.27.py

Key JOINT logic (builder v4):
- Uses hyd_injection tags to overwrite coupled coefficients in:
    prep, hyd (C3C4), stageD

IMPORTANT DIFFERENCE vs your fgonly runner:
- The JOINT builder v4 does NOT create the extra "stoich water" technosphere exchange in the hydrolysis node.
  Therefore this runner does NOT search for or update hyd_water exchanges.

Uncertainty layers (CLI: --unc-layer):
  - fgonly : sample ONLY foreground params; use_distributions=False
  - bgonly : sample ONLY exchange uncertainty; use_distributions=True
...
# Legacy aliases builder may create:
def legacy_c3c4_candidates(sid: str) -> List[str]:
    return [c3c4_code_for(sid), f"al_hydrolysis_treatment_CA__{sid}", f"al_hydrolysis_treatment_CA_GATE__{sid}"]

def legacy_stageD_candidates(sid: str) -> List[str]:
    return [stageD_code_for(sid), f"al_hydrolysis_stageD_offsets_CA__{sid}"]
...
def _sigma_from_gm_p5_p95(gm: float, p5: float, p95: float) -> float:
    ...
    return max(0.5 * (s_hi + s_lo), 0.0)

def sample_lognormal_gm_p5_p95(rng: np.random.Generator, spec: LognormalSpec) -> float:
    gm = max(float(spec.gm), 1e-30)
    sigma = _sigma_from_gm_p5_p95(spec.gm, spec.p5, spec.p95)
    if sigma <= 0:
        return gm
    mu = math.log(gm)
...
# =============================================================================
# Hydrolysis chemistry constants (for derived coefficients)
# =============================================================================
...
# =============================================================================
# Injection handles (tag-first, JOINT builder v4)
# =============================================================================
...
def build_hydrolysis_handles(
    ...
    *,
    ...
    prep = pick_activity_by_code_candidates(
        fg_db, [prep_code_for(sid)], logger, label=f"{sid} :: prep", fallback_search=f"scrap preparation hydrolysis {sid}"
    )

    # Tagged injection exchanges (JOINT builder v4)
    ex_prep_scrap_in = _find_unique_exchange_by_tag(prep, tag_key="hyd_injection", tag_val="prep_gate_scrap_in_per_kg_prepared")
    ex_hyd_prep = _find_unique_exchange_by_tag(hyd, tag_key="hyd_injection", tag_val="hyd_prepared_mass_per_kg_gate")
    ex_hyd_electrolyte = _find_unique_exchange_by_tag(hyd, tag_key="hyd_injection", tag_val="hyd_electrolyte_makeup_kg_per_kg_gate")
    ex_hyd_ww = _find_unique_exchange_by_tag(hyd, tag_key="hyd_injection", tag_val="hyd_purge_m3_per_kg_gate")
    ex_hyd_psa = _find_unique_exchange_by_tag(hyd, tag_key="hyd_injection", tag_val="hyd_psa_service_kg_per_kg_gate")
    ex_hyd_elec = _find_unique_exchange_by_tag(hyd, tag_key="hyd_injection", tag_val="hyd_electricity_total_kwh_per_kg_gate")

    ex_stageD_h2 = _find_unique_exchange_by_tag(stageD, tag_key="hyd_injection", tag_val="stageD_h2_credit_kg_per_kg_gate")
    ex_stageD_aloh3 = _find_unique_exchange_by_tag(stageD, tag_key="hyd_injection", tag_val="stageD_aloh3_credit_kg_per_kg_gate")
...
def apply_fg_sample(h: HydrolysisInjHandles, s: HydrolysisFgSample) -> Dict[str, float]:
    ...
    prepared_mass = Y_prep
    al_feed = prepared_mass * f_Al
    reacted_al = al_feed * X_Al

    electrolyte_makeup_kg = L * 1.0 * f_makeup * al_feed
    purge_m3 = (L * f_makeup * al_feed) / 1000.0

    h2_crude = yield_h2_kg_per_kg_al() * reacted_al
    h2_usable = R_PSA * h2_crude
    aloh3 = yield_aloh3_kg_per_kg_al() * reacted_al

    elec_total_kwh = (E_aux + E_therm) * prepared_mass

    # Write updates
    h.ex_prep_scrap_in["amount"] = 1.0 / max(Y_prep, 1e-12); h.ex_prep_scrap_in.save()

    h.ex_hyd_prep["amount"] = prepared_mass; h.ex_hyd_prep.save()
    h.ex_hyd_electrolyte["amount"] = electrolyte_makeup_kg; h.ex_hyd_electrolyte.save()
    h.ex_hyd_ww["amount"] = purge_m3; h.ex_hyd_ww.save()
    h.ex_hyd_psa["amount"] = h2_crude; h.ex_hyd_psa.save()
    h.ex_hyd_elec["amount"] = elec_total_kwh; h.ex_hyd_elec.save()

    h.ex_stageD_h2["amount"] = -h2_usable; h.ex_stageD_h2.save()
    h.ex_stageD_aloh3["amount"] = -aloh3; h.ex_stageD_aloh3.save()

    return {
        "prepared_mass_per_kg_gate": prepared_mass,
        "al_feed_kg_per_kg_gate": al_feed,
        "al_reacted_kg_per_kg_gate": reacted_al,
        "electrolyte_makeup_kg_per_kg_gate": electrolyte_makeup_kg,
        "purge_m3_per_kg_gate": purge_m3,
        "h2_crude_kg_per_kg_gate": h2_crude,
        "h2_usable_kg_per_kg_gate": h2_usable,
        "aloh3_kg_per_kg_gate": aloh3,
        "elec_total_kwh_per_kg_gate": elec_total_kwh,
    }
...
def build_case_demands_ids(c3c4_act, stageD_act, fu_kg: float) -> Dict[str, Dict[int, float]]:
    fu = float(fu_kg)
    return {
...
def build_case_demands_obj(c3c4_act, stageD_act, fu_kg: float) -> Dict[str, Dict[Any, float]]:
    fu = float(fu_kg)
    return {
        "c3c4": {c3c4_act: fu},
...
def run_monte_carlo(
    *,
    ...
    for sid, h in handles_by_sid.items():
        demands_ids = build_case_demands_ids(h.hyd_act, h.stageD_act, fu_kg)
...
def main():
    ...
    ap.add_argument("--tag", default="prospect_hydrolysis_joint")
    ...
    logger = setup_logger("run_hydrolysis_prospect_lcia_midpointH_joint_v1")
    ...
    for sid in scenario_ids:
        ...
        d = build_case_demands_obj(h.hyd_act, h.stageD_act, float(args.fu_al_kg))
        for case, dem in d.items():
            det_demands[(sid, case)] = dem
    ...
    logger.info("[done] Prospective hydrolysis JOINT uncertainty LCIA run complete (v1).")
