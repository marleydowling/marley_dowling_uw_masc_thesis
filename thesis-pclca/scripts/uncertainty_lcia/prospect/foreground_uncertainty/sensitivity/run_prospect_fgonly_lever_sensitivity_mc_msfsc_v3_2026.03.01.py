# -*- coding: utf-8 -*-
"""
run_prospect_fgonly_lever_sensitivity_mc_msfsc_v3_2026.03.01.py
...
-----------
A) Robust LCA init:
   - Attempt bw.prepare_lca_inputs() if available
   - Try bc.LCA(..., data_objs=...)
   - If TypeError "Unknown input type for loading datapackage: <class 'dict'>", FALL BACK to classic bc.LCA

B) f_transition baseline=0:
   - If route_c3c4<-fscB exchange is 0, bw2calc can prune it from sparse matrix.
   - Insert an epsilon CSR entry so the (row,col) exists and can be patched.
...
class PatchHandle:
    ...
    col: int
    data_index: int
    multiplier: float  # use -1 for technosphere conventions here
...
def build_patch_handle(lca: Any, A_csr, *, consumer: Any, provider: Any, name: str, logger: logging.Logger) -> PatchHandle:
    ...
    mult = -1.0  # your FG convention: A entry is negative of exchange amount

    logger.info(
        "[patch] %-34s | consumer=%s | provider=%s | (row=%d,col=%d,idx=%d) mult=%+.1f",
...
# =============================================================================
# LCA init (robust: try datapackages, but fall back if dict-like)
# =============================================================================
...
def init_lca(demand: Dict[Any, float], method: Tuple[str, ...], *, include_bg_unc: bool, seed: int, logger: logging.Logger) -> Any:
    ...
    # Try prepare_lca_inputs path, but *only* keep it if bc.LCA succeeds
    if hasattr(bw, "prepare_lca_inputs"):
        try:
            ...
            if isinstance(prepared, tuple) and len(prepared) == 2:
                ...
                data_objs, remapping_dicts = prepared, None

            try:
                lca = bc.LCA(demand, method=method, data_objs=data_objs, remapping_dicts=remapping_dicts, **kwargs)
                lca.lci()
                lca.lcia()
                logger.info("[lca] initial score=%g (used_prepare=True)", float(getattr(lca, "score", float("nan"))))
                return lca
            except TypeError as e:
                msg = str(e)
                if "Unknown input type for loading datapackage" in msg and "dict" in msg:
                    logger.warning("[prep] data_objs incompatible with bw2calc (dict-like). Falling back to classic bc.LCA.")
                else:
                    raise
        except Exception as e:
            logger.warning("[prep] prepare_lca_inputs path failed; falling back to classic bc.LCA: %s", e)

    # Classic init
    lca = bc.LCA(demand, method=method, **kwargs)
    lca.lci()
    lca.lcia()
    logger.info("[lca] initial score=%g (used_prepare=False)", float(getattr(lca, "score", float("nan"))))
    return lca
...
def run_one_scenario(
    *,
    ...
    # Ensure CSR matrix and attach to LCA (we patch this in-place)
    ...
    # Ensure patch slot exists for route_c3c4<-fscB (central may be 0)
    ...
    if inserted:
        ...
        A = lca.technosphere_matrix.tocsr()
        logger.info("[msfsc] Inserted epsilon CSR entry for route_c3c4<-fscB (row=%d,col=%d).", row_fb, col_fb)
    ...
    def _find_provider_in(act: Any, needles: List[str]) -> Any:
        ...
        for exc in act.exchanges():
            ...
            if any(n in blob for n in needles):
                return inp
        raise KeyError(f"Could not find provider in {act.key} for {needles}")
    ...
    def _recalc() -> float:
        # safer: use redo if available (tends to respect patched matrices)
        try:
