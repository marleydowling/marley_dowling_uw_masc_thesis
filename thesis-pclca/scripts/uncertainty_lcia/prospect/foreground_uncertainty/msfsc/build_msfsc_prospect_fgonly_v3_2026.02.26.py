# -*- coding: utf-8 -*-
"""
build_msfsc_prospect_fgonly_v3_2026.02.26.py
...
def purge_dangling_exchanges(fg_db_name: str, logger: logging.Logger, *, apply: bool) -> int:
    ...
    for act in fg:
        for exc in list(act.exchanges()):
            ...
            # If input is a dataset in a DB that doesn't exist in this project, drop it
...
def build_stageD_credit_wrapper(
    ...
    *,
    ...
    logger.info(f"[stageD-wrap] {act.key} | avoided={ingot_proxy.key} | sub_ratio={sub_ratio}")
    return act
...
def build_one_scenario(
    *,
    ...
    if apply and gateA is not None:
        ...
        logger.info(f"[gateA] {gateA.key} | removed_routing={removed_routing} | removed_hidden={removed_hidden}")
    ...
    stageD = build_stageD_credit_wrapper(
        ...
        ingot_proxy=ing_inert if (apply and ing_inert is not None) else ingot_bg,
...
def main() -> None:
    ...
    inj_all = {
        "builder": "build_msfsc_prospect_fgonly_v3_2026.02.26",
