# -*- coding: utf-8 -*-
"""
build_hydrolysis_prospect_fgonly_v4_2026.02.27.py  (REVISED)

Aluminium Hydrolysis (PROSPECTIVE 2050; multi-background SSPs) — FG-only *rebuild* builder (with PURGE)
======================================================================================================

What changed vs v2 (this revision)
----------------------------------
- Adds optional stripping of embedded negative technosphere "market outputs" that are NOT waste/scrap:
    --strip-nonwaste-negmarkets (default=1)
  Applied ONLY to BG-cloned proxies in the C3–C4 chain:
    scrap_gate, di, ww, psa, naoh_proxy

  Purpose:
  - Reduce embedded coproduct-credit leakage in C3–C4 chain so that Stage D is the only
    deliberate credit mechanism, matching deterministic modeling intent.
  - Keeps waste-like and scrap-like negative outputs intact.

Everything else
---------------
- Purge behavior unchanged
- Gate-basis wiring unchanged
- Stage D retains exactly 2 negative technosphere exchanges
- Legacy aliases unchanged
- First-layer localization unchanged
"""
...
# Used for "Gate A routing removal"
...
class HydrolysisCentral2050:
    ...
    # Electrolyte recipe (kept deterministic)
...
# =============================================================================
# NEW: Strip non-waste negative market outputs (embedded coproduct credits)
# =============================================================================
...
def strip_nonwaste_negative_market_outputs(act: Any, logger: logging.Logger, *, label: str) -> int:
    """
    Remove negative technosphere exchanges where:
      - amount < 0
      - input name is market/market group
      - NOT waste-like and NOT scrap-like
    Keep:
      - negative waste outputs (treatment, disposal, wastewater, etc.)
      - negative scrap outputs
      - electricity markets (ignored)
    """
    removed = 0
    kept_waste = 0
    kept_scrap = 0
...
def build_one_scenario(
    ...
    *,
    ...
    # -------------------------------------------------------------------------
    # 2) Prep activity (per kg prepared scrap output)
    # -------------------------------------------------------------------------
    ...
    if apply and prep is not None:
        add_tech(prep, scrap_gate, 1.0 / float(p.y_prep), unit="kilogram",
                comment="INJ:Y_prep -> gate_scrap_in_per_kg_prepared = 1/Y_prep")
        add_tech(prep, elec_bundle["mv"], float(p.prep_elec_kwh_per_kg_prepared), unit="kilowatt hour",
                comment="DET: prep electricity per kg prepared (fixed baseline)")

    # -------------------------------------------------------------------------
    # 3) Utility / proxy nodes cloned from BG (FG-only)
    # -------------------------------------------------------------------------
    ...
    if apply and di is not None:
        ...
        if strip_nonwaste_negmarkets:
            strip_nonwaste_negative_market_outputs(di, logger, label="di_water")
    ...
    # -------------------------------------------------------------------------
    # 4) CENTRAL coefficients (runner injection points)
    # -------------------------------------------------------------------------
    prepared_mass_per_kg_gate = float(p.y_prep)  # kg prepared per kg gate
    al_feed_kg_per_kg_gate = prepared_mass_per_kg_gate * float(p.f_al)
    ...
    inj = {
        ...
        "options": {"strip_nonwaste_negmarkets": bool(strip_nonwaste_negmarkets)},
...
def main() -> None:
    ...
    if apply and args.purge_only:
        ...
        inj_all: Dict[str, Any] = {
            "builder": "build_hydrolysis_prospect_fgonly_v2_2026.02.26 (revised)",
