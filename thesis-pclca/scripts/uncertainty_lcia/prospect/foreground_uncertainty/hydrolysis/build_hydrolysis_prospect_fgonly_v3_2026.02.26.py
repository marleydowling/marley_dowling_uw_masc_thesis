# -*- coding: utf-8 -*-
"""
build_hydrolysis_prospect_fgonly_v2_2026.02.26.py

Aluminium Hydrolysis (PROSPECTIVE 2050; multi-background SSPs) — FG-only *rebuild* builder (with PURGE)
======================================================================================================

What changed vs v1
------------------
1) Adds an APPLY-time purge step (mirrors your MSFSC v3 fix):
   - Clears exchanges on legacy "bad-code" FG activities (e.g., codes containing MYOP or "__prospective_conseq_IMAGE_...")
     and leaves a single self-production exchange, so BW processing won't crash.
   - Deletes dangling exchanges anywhere in the FG DB whose inputs can't be resolved (UnknownObject) OR point to a
     database that doesn't exist in the project (e.g., *_MYOP).

2) After apply, processes the FG DB to fail-fast if anything is still dangling.
...
Dry run (default; no writes):
  python build_hydrolysis_prospect_fgonly_v2_2026.02.26.py

Apply rebuild (writes + purge + process):
  python build_hydrolysis_prospect_fgonly_v2_2026.02.26.py --apply

Purge-only (writes purge + process, no rebuild):
  python build_hydrolysis_prospect_fgonly_v2_2026.02.26.py --apply --purge-only

Notes
-----
- Apply-safe rebuild semantics for *the nodes this script owns*: CLEAR + rewrite exchanges (no BAK proliferation).
- Purge step is intentionally broad to prevent 'databases.clean()' failures in runners.
...
# =============================================================================
# PURGE (fixes MYOP/dangling exchanges that break databases.clean / process)
# =============================================================================

def _safe_code(act: Any) -> str:
    c = act.get("code") if hasattr(act, "get") else None
    if c:
        return str(c)
...
def _is_legacy_bad_code(code: str) -> bool:
    c = (code or "")
    c_low = c.lower()
    for s in LEGACY_BAD_CODE_SUBSTRINGS:
        if s.lower() in c_low:
            return True
    return False


def purge_legacy_badcode_activities(
    fg: bw.Database,
    *,
    logger: logging.Logger,
    apply: bool,
    ...
    """
    Clear all exchanges on activities whose CODE looks like legacy bad suffixing (MYOP, db-name-in-code, etc).
    Leave a single production exchange to self so processing won't fail.
    """
    if not apply:
        logger.info("[purge] (dry) would scan and clear legacy bad-code activities")
        return 0
    ...
    for act in fg:
        ...
        if not code:
            continue
        if _is_legacy_bad_code(code):
            unit = act.get("unit") or "kilogram"
            clear_exchanges(act)
            ensure_single_production(act, unit)
            act["comment"] = (act.get("comment") or "") + "\nPURGED: legacy bad-code activity; exchanges cleared to prevent dangling MYOP references."
            act.save()
            n += 1

    logger.info(f"[purge] legacy bad-code activities cleared: {n}")
    return n


def purge_dangling_exchanges_anywhere(
    fg: bw.Database,
    *,
    logger: logging.Logger,
    apply: bool,
    ...
    This is the key fix for errors like:
      Exchange between ('..._MYOP', '...') and ('mtcw_foreground...', '...__..._MYOP') is invalid
    """
    if not apply:
        logger.info("[purge] (dry) would scan and delete dangling exchanges in FG DB")
    ...
    for act in fg:
        for exc in list(act.exchanges()):
            ...
            try:
                ...
                # safest: delete; otherwise processing can still explode
                exc.delete()
                deleted += 1
                unknown_input += 1
                continue
    ...
    logger.info(
        f"[purge] dangling exchanges removed: deleted={deleted} | unknown_input={unknown_input} | missing_db={missing_db} | bad_key={bad_key}"
    )
    return {"deleted": deleted, "unknown_input": unknown_input, "missing_db": missing_db, "bad_key": bad_key}


def purge_fg_db(fg: bw.Database, *, logger: logging.Logger, apply: bool) -> None:
    purge_legacy_badcode_activities(fg, logger=logger, apply=apply)
    purge_dangling_exchanges_anywhere(fg, logger=logger, apply=apply)
...
def build_one_scenario(
    ...
    *,
    ...
    if apply and di is not None:
        clone_nonprod_exchanges(
            src=di_src, tgt=di, scen_id=scen_id, scale=1.0,
            elec_bundle=elec_bundle, util_map=util,
    ...
    if apply and ww is not None:
        clone_nonprod_exchanges(
            src=ww_src, tgt=ww, scen_id=scen_id, scale=1.0,
            elec_bundle=elec_bundle, util_map=util,
            drop_techno_ref_products=None, logger=logger, apply=True
        )
    ...
    if apply and psa is not None:
        clone_nonprod_exchanges(
            src=psa_src, tgt=psa, scen_id=scen_id, scale=1.0,
            elec_bundle=elec_bundle, util_map=util,
            drop_techno_ref_products=None, logger=logger, apply=True
        )
    ...
    if apply and naoh_proxy is not None:
        clone_nonprod_exchanges(
            src=naoh_src, tgt=naoh_proxy, scen_id=scen_id, scale=1.0,
            elec_bundle=elec_bundle, util_map=util,
            drop_techno_ref_products=None, logger=logger, apply=True
        )
    ...
    # -------------------------------------------------------------------------
    # 4) CENTRAL coefficients (all are injection points for runner)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 5) Hydrolysis C3–C4 node (GATE BASIS)
    # -------------------------------------------------------------------------
    hyd = upsert_fg_activity(
        ...
        comment=(
            ...
            "Injection points (runner): Y_prep, f_Al, X_Al, L, f_makeup, E_aux, E_therm.\n"
            "Aggregated electricity is ONE exchange.\n"
        ),
        logger=logger,
        apply=apply,
    )
    if apply and hyd is not None:
        add_tech(hyd, prep, prepared_mass_per_kg_gate, unit="kilogram", comment="INJ:Y_prep -> prepared_mass_per_kg_gate = Y_prep")
        add_tech(hyd, electrolyte, electrolyte_makeup_kg_per_kg_gate, unit="kilogram", comment="INJ:L,f_makeup,Y_prep,f_Al -> electrolyte_makeup")
        add_tech(hyd, ww, purge_m3_per_kg_gate, unit="cubic meter", comment="INJ:L,f_makeup,Y_prep,f_Al -> purge_m3")
        if stoich_makeup_water_kg_per_kg_gate > 0:
            add_tech(hyd, water_provider_for_electrolyte, stoich_makeup_water_kg_per_kg_gate, unit="kilogram", comment="INJ:X_Al,Y_prep,f_Al -> stoich_makeup_water")
        add_tech(hyd, psa, h2_crude_kg_per_kg_gate, unit="kilogram", comment="INJ:X_Al,Y_prep,f_Al -> PSA service demand = H2_crude")
        add_tech(hyd, elec_bundle["mv"], elec_total_kwh_per_kg_gate, unit="kilowatt hour", comment="INJ:E_aux,E_therm,Y_prep -> hydrolysis electricity (aggregated)")

    # -------------------------------------------------------------------------
    # 6) Receiving market proxies + optional first-layer localization
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 7) Stage D credit-only node (GATE BASIS)
    # -------------------------------------------------------------------------
    ...
    if apply and stageD is not None:
        ...
        add_tech(stageD, h2_proxy, -float(h2_usable_kg_per_kg_gate), unit="kilogram", comment="INJ:R_PSA,X_Al,Y_prep,f_Al -> H2 usable credit")
        add_tech(stageD, aloh3_proxy, -float(aloh3_kg_per_kg_gate), unit="kilogram", comment="INJ:X_Al,Y_prep,f_Al -> Al(OH)3 credit")
...
def parse_args() -> argparse.Namespace:
    ...
    # purge controls
    ap.add_argument("--no-purge", action="store_true", help="Skip purge step (NOT recommended).")
    ap.add_argument("--purge-only", action="store_true", help="Run purge + process only; skip rebuild of hydrolysis nodes.")
...
def main() -> None:
    args = parse_args()
    root = get_root_dir()
    logger = setup_logger(root, "build_hydrolysis_prospect_fgonly_v2")
    ...
    # Purge first (apply mode), so rebuild doesn't inherit any bad legacy edges
    if apply and (not args.no_purge):
        logger.info("[purge] Running FG DB purge (legacy bad-code + dangling exchanges)...")
        purge_fg_db(fg, logger=logger, apply=True)
    elif apply and args.no_purge:
        logger.warning("[purge] SKIPPED due to --no-purge (this can reintroduce MYOP/dangling failures).")

    if apply and args.purge_only:
        logger.info("[purge-only] Skipping rebuild; proceeding to process FG DB.")
    else:
        p = HydrolysisCentral2050()
        logger.info("[params] hydrolysis central 2050: %s", asdict(p))

        inj_all: Dict[str, Any] = {
            "builder": "build_hydrolysis_prospect_fgonly_v2_2026.02.26",
