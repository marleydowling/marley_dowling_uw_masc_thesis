# -*- coding: utf-8 -*-
"""
run_hydrolysis_prospect_lcia_midpointH_fgonly_v2_2026.02.26.py
...
v2 fixes vs v1
--------------
1) Purge-aware picking:
   - Filters out legacy bad-code activities (MYOP / DB-name-in-code substrings) during fallback search.
   - Filters out "purged" activities (no technosphere exchanges) during fallback search.
   - Optional require_technosphere gate on picks where needed.

2) LCIA method switching correctness:
   - When --mc-all-methods is used, ensures each case is scored under the intended method
     (switch back to primary before scoring primary).

3) rho (liquor density) inferred from built central coefficient:
   - electrolyte_makeup = L * rho * f_makeup * (Y_prep*f_Al)
   - rho inferred per scenario from central hydrolysis electrolyte coefficient (should be ~1.0)

Uncertainty layers (CLI: --unc-layer):
  - fgonly : Sample ONLY foreground parameters (PERT/lognormal) and write updated exchange amounts into FG DB; background fixed.
  - bgonly : Sample ONLY background exchange uncertainty (ecoinvent distributions) via bw2calc use_distributions=True; FG fixed at central.
...
- Each iteration builds a fresh LCA for that scenario (seeded per-iteration) so BG draws are consistent across cases within an iteration.
...
# =============================================================================
# PURGE-AWARE FILTERS (must match builder's purge substrings)
# =============================================================================
...
def _is_legacy_bad_code(act: Any) -> bool:
    code = (_safe_code(act) or "")
    c_low = code.lower()
...
def pick_activity_by_code_candidates(
    ...
    *,
    ...
    # Try explicit codes first (these should not be legacy bad-code)
    for c in codes:
        act = _try_get_by_code(db, c)
        if act is None:
            continue
        if _is_legacy_bad_code(act):
            logger.warning(f"[pick][skip] {label}: code hit is legacy-bad-code -> {_safe_code(act)}")
            continue
        if require_technosphere and (not _has_any_technosphere(act)):
            logger.warning(f"[pick][skip] {label}: code hit has no technosphere (purged?) -> {_safe_code(act)}")
            continue
        logger.info(f"[pick] {label}: {act.key} loc={act.get('location')} code={_safe_code(act)} name='{act.get('name')}'")
        return act
    ...
    if not filtered:
        raise RuntimeError(
            f"Could not resolve {label}; all search hits were filtered out as legacy-bad/purged. search='{fallback_search}'"
        )
...
# =============================================================================
# Injection handles (exchange objects)
# =============================================================================

@dataclass
class HydrolysisInjHandles:
    sid: str
    rho: float  # inferred liquor density used in electrolyte_makeup mapping
...
def build_hydrolysis_handles(
    ...
    *,
    ...
    hyd = pick_activity_by_code_candidates(
        fg_db, legacy_c3c4_candidates(sid), logger, label=f"{sid} :: hydrolysis C3C4",
        fallback_search=f"al hydrolysis treatment {sid}",
        require_technosphere=True,
    )
    stageD = pick_activity_by_code_candidates(
        fg_db, legacy_stageD_candidates(sid), logger, label=f"{sid} :: hydrolysis StageD",
        fallback_search=f"stage d offsets hydrolysis {sid}",
        require_technosphere=True,
    )

    prep = pick_activity_by_code_candidates(
        fg_db, [prep_code_for(sid)], logger, label=f"{sid} :: prep",
        fallback_search=f"shredding preparation hydrolysis {sid}",
        require_technosphere=True,
    )
    scrap_gate = pick_activity_by_code_candidates(
        fg_db, [scrap_gate_code_for(sid)], logger, label=f"{sid} :: scrap_gate",
        fallback_search=f"scrap post-consumer at gate {sid}",
        require_technosphere=True,
    )
    electrolyte = pick_activity_by_code_candidates(
        fg_db, [electrolyte_code_for(sid)], logger, label=f"{sid} :: electrolyte",
        fallback_search=f"naoh electrolyte solution {sid}",
        require_technosphere=True,
    )
    ww = pick_activity_by_code_candidates(
        fg_db, [ww_code_for(sid)], logger, label=f"{sid} :: wastewater",
        fallback_search=f"wastewater treatment {sid}",
        require_technosphere=True,
    )
    psa = pick_activity_by_code_candidates(
        fg_db, [psa_code_for(sid)], logger, label=f"{sid} :: psa",
        fallback_search=f"pressure swing adsorption {sid}",
        require_technosphere=True,
    )
    h2p = pick_activity_by_code_candidates(
        fg_db, [h2_proxy_code_for(sid)], logger, label=f"{sid} :: h2_proxy",
        fallback_search=f"hydrogen low pressure proxy {sid}",
        require_technosphere=True,
    )
    aoh = pick_activity_by_code_candidates(
        fg_db, [aloh3_proxy_code_for(sid)], logger, label=f"{sid} :: aloh3_proxy",
        fallback_search=f"aluminium hydroxide proxy {sid}",
        require_technosphere=True,
    )

    # electricity exchange on hydrolysis
    elec_exc = None
    ...
    # water exchange on hydrolysis (stoich makeup water)
    water_exc = None
    for exc in hyd.exchanges():
        if exc.get("type") != "technosphere":
            continue
        ...
        if ("water" in rp) or nm.startswith("market for tap water") or nm.startswith("water production"):
            # prefer explicit stoich comment if present
    ...
    # Infer rho from the central electrolyte coefficient:
    # coeff_e = L0 * rho * f_makeup0 * (Y0*f_Al0)
    L0 = float(spec.L.gm)
    fmk0 = float(spec.f_makeup.mode)
    Y0 = float(spec.Y_prep.mode)
    fAl0 = float(spec.f_Al.mode)
    denom = L0 * fmk0 * (Y0 * fAl0)
    rho_hat = (central_amounts["hyd_electrolyte"] / denom) if denom > 0 else 1.0
    logger.info(f"[rho] {sid}: inferred_rho={rho_hat:.6g} (central electrolyte->hyd={central_amounts['hyd_electrolyte']:.6g})")
...
def apply_fg_sample(h: HydrolysisInjHandles, s: HydrolysisFgSample) -> Dict[str, float]:
    # Derived coefficients per kg gate scrap treated (matches builder v2)
    ...
    stoich_makeup_water_kg_per_kg_gate = stoich_h2o_kg_per_kg_gate  # builder uses liquor_pool or separate_feed, both map to same mass

    working_liquor_L_per_kg_gate = L * al_feed_kg_per_kg_gate
    electrolyte_makeup_kg_per_kg_gate = working_liquor_L_per_kg_gate * rho * f_makeup
    ...
    h.ex_prep_scrap_in["amount"] = 1.0 / max(Y_prep, 1e-12); h.ex_prep_scrap_in.save()

    h.ex_hyd_prep["amount"] = prepared_mass_per_kg_gate; h.ex_hyd_prep.save()
    h.ex_hyd_electrolyte["amount"] = electrolyte_makeup_kg_per_kg_gate; h.ex_hyd_electrolyte.save()
    h.ex_hyd_ww["amount"] = purge_m3_per_kg_gate; h.ex_hyd_ww.save()
    h.ex_hyd_water["amount"] = stoich_makeup_water_kg_per_kg_gate; h.ex_hyd_water.save()
    h.ex_hyd_psa["amount"] = h2_crude_kg_per_kg_gate; h.ex_hyd_psa.save()
    h.ex_hyd_elec["amount"] = elec_total_kwh_per_kg_gate; h.ex_hyd_elec.save()

    h.ex_stageD_h2["amount"] = -h2_usable_kg_per_kg_gate; h.ex_stageD_h2.save()
    h.ex_stageD_aloh3["amount"] = -aloh3_kg_per_kg_gate; h.ex_stageD_aloh3.save()
...
def run_mc_one_scenario(
    *,
    ...
    for it in range(1, iterations + 1):
        ...
        # Init LCA on primary method for first case
        case0 = cases[0]
        ...
        for case in cases:
            if case != case0:
                ...
                # Ensure we are on primary before primary scoring
                if getattr(lca, "method", None) != primary_method:
                    lca.switch_method(primary_method)
                lca.redo_lcia()

            # Always score each requested method explicitly (avoids wrong-method bug)
            for m in selected_methods:
                if getattr(lca, "method", None) != m:
                    lca.switch_method(m)
                lca.redo_lcia()
                score = float(lca.score)
...
def main():
    ...
    for sid in scenario_ids:
        h = build_hydrolysis_handles(fg_db, sid=sid, logger=logger, spec=spec)
