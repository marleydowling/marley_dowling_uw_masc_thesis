...
- Adds uncertainty carryover + correct scaling:
  * When cloning exchanges from BG->FG, copy uncertainty fields:
      'uncertainty type', 'loc', 'scale', 'shape', 'minimum', 'maximum', 'pedigree', 'formula', 'comment', 'negative'
  * When scaling exchange amounts (prep_scale, degrease_scale), uncertainty params are rescaled consistently:
      - lognormal: sigma unchanged; loc reset so mean matches |amount|
      - normal/uniform/triangular: loc/scale/min/max scale linearly
  * Swaps of electricity/utilities preserve the uncertainty fields already on those exchanges.

Overwrite semantics:
- --apply without --overwrite will NOT wipe/replace existing FG activities (safe-by-default).
- With --overwrite, activities are cleared and rebuilt.
...
Usage:
  # DRY RUN audit
  python ...\build_msfsc_prospect_uncertainty_v2_2026.02.28.py

  # APPLY + overwrite
  python ...\build_msfsc_prospect_uncertainty_v2_2026.02.28.py --apply --overwrite
"""
...
def electricity_loc_preference(primary: Optional[str] = None) -> List[str]:
    ...
    Electricity policy:
    - NEVER include 'CA-*'
    - If primary is a province token (CA-XX), allow fallback to CA aggregate only.
    """
    pref: List[str] = []
    if primary:
        pref.append(primary)
...
# =============================================================================
# Uncertainty helpers (counts + copy + rescale)
# =============================================================================
...
def _has_uncertainty(exc) -> bool:
    """
    True if exchange has stochastic uncertainty.
    Treat UTYPE_UNDEFINED (0) and UTYPE_NONE (1) as deterministic.
    """
    ut = _utype_int(exc)
    if ut is None:
        return False
...
def copy_uncertainty_metadata(src_exc, dst_exc) -> None:
    """Copy known-safe uncertainty/provenance fields from src exchange to dst exchange."""
...
def rescale_uncertainty_inplace(exc, factor: float, amount_new: float) -> None:
    ...
    Rules:
    - Lognormal: sigma ("scale") unchanged; set loc so mean == |amount_new|
        loc = ln(|amount_new|) - 0.5*sigma^2
      If sigma missing, set loc ~ ln(|amount_new|) when possible.
    ...
    if ut == UTYPE_LOGNORMAL:
        ...
        if sig is not None:
            ...
            # best-effort: keep mean aligned even if sigma missing
...
def clone_activity(
    ...
    *,
    ...
    """
    Clone src into FG:
    - If exists and overwrite=False: return as-is (no wiping)
    - Else: clear + copy non-production exchanges (with uncertainty) + single self production
    """
...
def make_or_clear(
    ...
    *,
    ...
    if key in fg_db and not overwrite:
        act = fg_db.get(code)
        if logger:
            logger.info("[make-skip]", key=act.key, overwrite=False)
        return act
...
def scale_all_exchanges(act: bd.Activity, factor: float) -> None:
    """
    Scale all technosphere + biosphere exchanges and rescale uncertainty metadata consistently.
    """
    f = float(factor)
    for exc in act.exchanges():
        if exc["type"] in ("technosphere", "biosphere"):
            ...
            exc["amount"] = amt1
            if _has_uncertainty(exc):
                rescale_uncertainty_inplace(exc, factor=f, amount_new=amt1)
            exc.save()


# =============================================================================
# MSFSC-specific helpers
# =============================================================================

def remove_matching_technosphere_inputs_any(act: bd.Activity, needles: List[str], logger=None) -> int:
    removed = 0
    needles_l = [(n or "").lower() for n in needles]
    for exc in list(act.exchanges()):
        if exc["type"] != "technosphere":
            continue
        inp = exc.input
        iname = (inp.get("name", "") or "").lower()
        if any(n in iname for n in needles_l):
            if logger:
                logger.info("[remove]", act=act.key, amt=float(exc["amount"]), inp=inp.key, loc=inp.get("location"), name=inp.get("name"))
            exc.delete()
            removed += 1
    return removed
...
def build_stageD_credit_wrapper(
    ...
    *,
    ...
    # If we didn't overwrite and it existed, don't double-add the exchange
    if overwrite:
...
def default_scenarios() -> Dict[str, str]:
    return {
...
def main():
    ...
    parser.add_argument("--overwrite", action="store_true", help="Rebuild/overwrite existing FG targets (recommended when BG has changed).")
    ...
    parser.add_argument("--scenario-ids", nargs="+", default=["SSP1VLLO_2050", "SSP2M_2050", "SSP5H_2050"])
    parser.add_argument("--bg-map", action="append", default=None, help="Map scenario to BG db: SSP2M_2050=db_name (repeatable)")
    ...
    for sid in scenario_ids:
        bg = bg_map.get(sid) or defaults.get(sid)
        if not bg:
    ...
    # liquid name candidates (robust across some premise/ei variants)
    liquid_candidates = [
        "aluminium production, primary, liquid, prebake",
        "aluminum production, primary, liquid, prebake",
        "aluminium production, primary, liquid",
        "aluminum production, primary, liquid",
    ]
    liquid_anchor = "aluminium production, primary, liquid"  # used for rewire detection
    ...
    if not args.apply:
        ...
        for scenario_label, bg_db in scenarios:
            ...
            # liquid (try candidates; allow_contains=True as last resort)
            liquid_bg = None
            for nm in liquid_candidates:
                try:
                    liquid_bg = pick_activity(bg_db, nm, preferred_locs=credit_al_loc_pref, allow_contains=True, logger=logger, kind="pick-credit")
                    break
                except Exception:
                    continue
            if liquid_bg is None:
                raise KeyError(f"Could not resolve any liquid template candidates in BG='{bg_db}': {liquid_candidates}")
    ...
    for scenario_label, bg_db in scenarios:
        ...
        gateA = clone_activity(
            prep_tpl, args.fg_db, gateA_code, gateA_name,
        ...
        if args.overwrite:
            ...
            # remove embedded routing to prepared-scrap market (robust match)
            routing_needles = [
                "market for aluminium scrap, post-consumer, prepared for melting",
                "market group for aluminium scrap, post-consumer, prepared for melting",
                "prepared for melting",
            ]
            removed_routing = remove_matching_technosphere_inputs_any(gateA, routing_needles, logger=logger)
        ...
        # deterministic add-on: explicit shredding electricity
        if args.overwrite:
        ...
        deg = clone_activity(
            deg_tpl, args.fg_db, deg_code, deg_name,
            location="CA", copy_uncertainty=True, overwrite=bool(args.overwrite), logger=logger
        )
        ...
        if args.overwrite:
            ...
            fsc_step.new_exchange(input=util_map["lubricating oil"], amount=float(fsc_lube_kg_per_kg_billet), type="technosphere").save()
        ...
        # Clone liquid baseline + inert and swap electricity to credit region
        liq_base_code = f"AL_primary_liquid_BASE_CA_{scenario_label}"
        liq_base = clone_activity(
            liquid_bg, args.fg_db, liq_base_code,
        ...
        liq_inert = clone_activity(
            liquid_bg, args.fg_db, liq_inert_code,
        ...
        ing_base = clone_activity(
            ingot_bg, args.fg_db, ing_base_code,
        ...
        ing_inert = clone_activity(
            ingot_bg, args.fg_db, ing_inert_code,
        ...
        stageD = build_stageD_credit_wrapper(
            args.fg_db, stageD_code, stageD_name, stageD_ingot,
        ...
        # --- Sanity QA for key nodes (always) ---
        for code in [
            ing_base_code,
            ing_inert_code,
            liq_base_code,
    ...
    logger.info("[done]", msg="All scenarios built with BG uncertainty carryover (v2).")

    # Quick post-apply audit
