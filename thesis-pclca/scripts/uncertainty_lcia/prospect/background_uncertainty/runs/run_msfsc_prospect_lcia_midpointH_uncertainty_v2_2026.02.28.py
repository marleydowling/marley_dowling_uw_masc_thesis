# -*- coding: utf-8 -*-
"""
run_msfsc_prospect_lcia_midpointH_uncertainty_v2_2026.02.28.py

Monte Carlo LCIA runner for PROSPECTIVE MS-FSC (2050 SSP backgrounds) that is SAFE-BY-DEFAULT
and aligned to:

  build_msfsc_prospect_uncertainty_v2_2026.02.28.py

This revision FIXES the logging crash you hit:
  TypeError: Logger._log() got an unexpected keyword argument 'project'

Root cause:
- stdlib logging.Logger does NOT accept keyword fields (that's structlog-style).
Fix:
- All logger calls use only f-strings / positional formatting (NO kwargs).

Key alignment with builder v2
----------------------------
- Strict node resolution by FG *code* (NO search fallback by default).
- Layer guards to avoid accidental mixing:
    * "bgonly" project should pair with "bgonly" FG DB
    * "_uncertainty_analysis" project should pair with "_uncertainty_analysis" FG DB
  Use --allow-mixed-layer only if you intentionally want to bypass this safety.

- Gate-basis FU handling matches builder:
    User provides FU as kg scrap at chain gate (GateA/Shred/Degrease basis).
    Builder defines: route_c3c4 (per 1 kg billet) demands gateA with amount scrap_per_billet.
...
- Stage D credit wrapper:
    MSFSC_stageD_credit_ingot_{variant}_CA_{SCEN}
- Optional NET wrappers (diagnostic):
    MSFSC_route_total_STAGED_NET_CA_{SCEN}
    MSFSC_route_total_UNITSTAGED_CA_{SCEN}

Cases
-----
- c3c4        : demand on route C3C4 wrapper
- stageD_only : demand on Stage D credit wrapper only (diagnostic)
- joint       : c3c4 + stageD_only (i.e., net = C3C4 + Stage D)
- net_wrapper : optional direct demand on chosen NET wrapper (diagnostic; requires --include-net-wrapper)
...
Optional:
- mc_samples_primary_<tag>_<ts>.csv                    (if --save-samples; streamed to disk)
...
- Does NOT add uncertainty to authored MSFSC parameters (yields, kWh, lube).
- Does NOT write to BW databases.

Example (uncertainty analysis layer)
------------------------------------
python run_msfsc_prospect_lcia_midpointH_uncertainty_v2_2026.02.28.py ^
...
# =============================================================================
# DEFAULTS (aligned to MSFSC builder v2)
# =============================================================================

DEFAULT_PROJECT = "pCLCA_CA_2025_prospective_uncertainty_analysis"
DEFAULT_FG_DB = "mtcw_foreground_prospective_uncertainty_analysis"
...
DEFAULT_OUT_DIR = DEFAULT_ROOT / "results" / "40_uncertainty" / "1_prospect" / "msfsc" / "uncertainty_analysis"
...
# Consider non-square solve fallback (common in exploratory FG DBs)
DEFAULT_ALLOW_LEAST_SQUARES_FALLBACK = True


# =============================================================================
# CODE PATTERNS (must match builder outputs)
# =============================================================================
...
def _workspace_root() -> Path:
    """
    If BRIGHTWAY2_DIR is set to ...\\brightway_base, treat its parent as the workspace root.
    Else fall back to DEFAULT_ROOT.
    """
...
# =============================================================================
# SAFETY GUARDS (layer consistency)
# =============================================================================

def _looks_bgonly(s: str) -> bool:
    s2 = (s or "").lower()
    return ("bgonly" in s2) or ("_bgonly" in s2)


def _looks_uncertainty_analysis(s: str) -> bool:
    s2 = (s or "").lower()
    return s2.endswith("_uncertainty_analysis") or ("_uncertainty_analysis" in s2)


def enforce_layer_guard(
    *,
    project: str,
    fg_db: str,
    allow_mixed: bool,
    ...
    """
    Prevent accidentally running in a mismatched layer (read-only runner, but wrong results are costly).
    FIXED: no logger kwargs.
    """
    if allow_mixed:
        logger.warning("[guard] allow_mixed_layer=True (bypassing layer guard).")
        return

    p_bg = _looks_bgonly(project)
    d_bg = _looks_bgonly(fg_db)
    p_unc = _looks_uncertainty_analysis(project)
    d_unc = _looks_uncertainty_analysis(fg_db)

    if p_bg and not d_bg:
        raise RuntimeError(
            "[guard] Refusing: project looks bgonly but fg-db does not.\n"
            f"        project='{project}'\n"
            f"        fg-db  ='{fg_db}'\n"
            "        (Use --allow-mixed-layer to override.)"
        )
    if d_bg and not p_bg:
        raise RuntimeError(
            "[guard] Refusing: fg-db looks bgonly but project does not.\n"
            f"        project='{project}'\n"
            f"        fg-db  ='{fg_db}'\n"
            "        (Use --allow-mixed-layer to override.)"
        )

    if p_unc and not d_unc:
        raise RuntimeError(
            "[guard] Refusing: project looks _uncertainty_analysis but fg-db does not.\n"
            f"        project='{project}'\n"
            f"        fg-db  ='{fg_db}'\n"
            "        (Use --allow-mixed-layer to override.)"
        )
    if d_unc and not p_unc:
        raise RuntimeError(
            "[guard] Refusing: fg-db looks _uncertainty_analysis but project does not.\n"
            f"        project='{project}'\n"
            f"        fg-db  ='{fg_db}'\n"
            "        (Use --allow-mixed-layer to override.)"
        )

    logger.info(f"[guard] layer pairing OK | project='{project}' | fg_db='{fg_db}'")


# =============================================================================
# PROJECT + DB + PICKERS (STRICT BY DEFAULT)
# =============================================================================
...
def pick_by_code_or_search(
    ...
    *,
    ...
    """
    If allow_search=False: strict code lookup only.
    If allow_search=True : allow db.search fallback (still logged as WARN).
    """
    ...
    if not allow_search:
        raise RuntimeError(
            f"Missing required activity for {label}.\n"
            f"  fg_db='{db.name}'\n"
            f"  required_code='{code}'\n"
            "  (Search fallback disabled; use --allow-search-fallback to enable.)"
        )
    ...
    best = hits[0]
    logger.warning(
        f"[pick][WARN] {label}: code-miss '{code}' -> fallback picked {best.key} "
        f"loc={best.get('location')} name='{best.get('name')}'"
    )
    return best
...
def detect_scrap_per_billet(route_c3c4_act, gateA_act, logger: logging.Logger) -> float:
    ...
    if best is None or best <= 0:
        raise RuntimeError(
            ...
            "Expected: MSFSC_route_C3C4_only has a technosphere input to MSFSC_gateA_DIVERT_PREP with amount>0."
        )

    logger.warning(f"[fu][WARN] Using fallback gateA-like coefficient: {best:.12g} kg_scrap_at_gateA per kg_billet")
    return best
...
def build_mc_lca_with_fallback(
    demand_ids_union: Dict[int, float],
    ...
    *,
    ...
    try:
        lca = bc.LCA(demand_ids_union, method, use_distributions=True, seed_override=seed)
        lca.lci()
        return lca
    except Exception as e:
        if allow_least_squares and _is_nonsquare_exception(e) and hasattr(bc, "LeastSquaresLCA"):
            logger.warning(f"[mc][lci][WARN] {type(e).__name__}: {e}")
            logger.warning("[mc][lci] Falling back to LeastSquaresLCA.")
            lca = bc.LeastSquaresLCA(demand_ids_union, method, use_distributions=True, seed_override=seed)  # type: ignore
            lca.lci()
            return lca
        raise
...
def run_monte_carlo(
    ...
    # Union demand used only to build indices (amounts arbitrary but non-zero).
    union_demand: Dict[int, float] = {}
    for d in demands_by_key_ids.values():
        for k in d.keys():
            union_demand[int(k)] = 1.0

    mc_lca = build_mc_lca_with_fallback(
        union_demand,
        primary_method,
        seed=seed,
    ...
    # Cache characterization matrices for selected methods
    c_mats: Dict[Tuple[str, str, str], Any] = {}
    for m in selected_methods:
        mc_lca.switch_method(m)
        c_mats[m] = mc_lca.characterization_matrix.copy()

    # Stream samples (primary only) to disk to avoid memory blowup
    samples_path = out_dir / f"mc_samples_primary_{tag}_{ts}.csv"
    samples_writer = None
    samples_fh = None
    if save_samples:
        samples_fh = samples_path.open("w", newline="", encoding="utf-8")
        samples_writer = csv.DictWriter(samples_fh, fieldnames=[
            "tag", "iteration", "scenario_id", "case", "method", "score"
        ])
        samples_writer.writeheader()
        logger.info(f"[mc-out] Samples CSV (primary only; streaming): {samples_path}")
        ...
        for it in range(1, iterations + 1):
            ...
            for (sid, case), demand_ids in demands_by_key_ids.items():
                ...
                for m in selected_methods:
                    ...
                    if samples_writer is not None and (m == primary_method):
...
def main():
    ...
    ap.add_argument("--no-least-squares-fallback", action="store_true",
                    help="Disable LeastSquaresLCA fallback on nonsquare technosphere.")
    ...
    logger = setup_logger("run_msfsc_prospect_uncertainty_midpointH_v2")

    enforce_layer_guard(
        project=str(args.project),
        fg_db=str(args.fg_db),
        allow_mixed=bool(args.allow_mixed_layer),
        logger=logger,
    )
    ...
    out_dir = Path(args.out_dir) if str(args.out_dir).strip() else (
        _workspace_root() / "results" / "40_uncertainty" / "1_prospect" / "msfsc"
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    logger.info("=" * 96)
    logger.info(f"[cfg] project={args.project} fg_db={args.fg_db}")
    ...
    logger.info(f"[strict] allow_search_fallback={bool(args.allow_search_fallback)}")
    logger.info("=" * 96)
    ...
    for sid in scenario_ids:
        gateA = pick_by_code_or_search(
            ...
            allow_search=bool(args.allow_search_fallback),
        ...
        # demands (object) for deterministic mode
        demands_obj[(sid, "c3c4")] = {route_c3c4: fu_billet}
        demands_obj[(sid, "stageD_only")] = {stageD: fu_billet}
        ...
        demands_ids[(sid, "stageD_only")] = {int(stageD.id): fu_billet}
    ...
    run_monte_carlo(
        ...
        out_dir=out_dir,
        tag=tag,
        allow_least_squares=(not bool(args.no_least_squares_fallback)),
        logger=logger,
    )

    logger.info("[done] Prospective MSFSC uncertainty LCIA run complete (v2).")
