...
Aluminium Hydrolysis (PROSPECTIVE 2050; multi-background SSPs) — BG-uncertainty carryover builder (layer-aware)
=============================================================================================================

What it does
------------
- DRY RUN (default): audits BG source uncertainty + existing FG targets (no DB writes).
- --apply: rebuilds Hydrolysis chain in the chosen uncertainty layer FG DB and COPIES uncertainty metadata
  from BG exchanges into FG clones (so MC sees BG exchange uncertainty at FG edges).

Layer targets (defaults)
------------------------
bgonly (default):
  project = pCLCA_CA_2025_prospective_unc_bgonly
  fg_db   = mtcw_foreground_prospective__bgonly
...
------
- DRY RUN by default.
- --apply is allowed only if (project, fg_db) are one of the known layer targets (above),
  unless you explicitly edit ALLOWED_PROJECTS/ALLOWED_FG_DBS.

Preserved v15 functionality
---------------------------
- Gate-basis hydrolysis per scenario:
    al_hydrolysis_treatment_CA_GATE_BASIS__{SCEN_ID}
- Gate-basis Stage D offsets per scenario:
    al_hydrolysis_stageD_offsets_CA_GATE_BASIS__{SCEN_ID}
- Gate A intent: demand on C3C4 == 1 kg scrap at Gate A
- Drops embedded prepared-scrap routing at Gate A (drop specific ref product exchange)
- Scenario-specific BG electricity bundle selection + utility provider selection
- Optional first-layer localization of receiving systems (H2 + Al(OH)3)
- Writes legacy aliases as pass-through to prevent wrong-basis execution

Outputs
-------
- Logs: <workspace_root>/logs/
- Reports: <workspace_root>/results/uncertainty_audit/hydrolysis_prospect/<layer>/
    - missing_uncertainty_exchanges_<ts>.csv
    - uncertainty_coverage_summary_<ts>.csv
- Registry:
    <workspace_root>/scripts/90_database_setup/uncertainty_assessment/activity_registry__hydrolysis_prospect_bg_uncertainty.json
"""
...
# =============================================================================
# Layer targets (align with your other uncertainty scripts)
# =============================================================================
...
DEFAULT_PROJECT_NAME = DEST_PROJECTS[DEFAULT_LAYER]
DEFAULT_FOREGROUND_DB_NAME = DEST_FG_DB[DEFAULT_LAYER]

DEFAULT_ROOT = Path(r"C:\brightway_workspace")


# =============================================================================
# Default scenarios (match your prospective PERF DB naming)
# =============================================================================
...
def setup_logger(name: str = "hydrolysis_prospect_bg_uncertainty") -> logging.Logger:
...
def registry_path() -> Path:
    ...
    return (
        ...
        / "activity_registry__hydrolysis_prospect_bg_uncertainty.json"
    )
...
def load_registry(path: Path, logger: logging.Logger) -> Dict[str, Any]:
    ...
    logger.info(f"[registry] USE=True path={path}")
    return {"version": "hydrolysis_prospect_bg_uncertainty_v2", "records": {}}
...
def report_dir(layer: str) -> Path:
    root = _workspace_root()
    out = root / "results" / "uncertainty_audit" / "hydrolysis_prospect" / layer
...
# =============================================================================
# Helpers: scenario suffix + location scoring + selection
# =============================================================================
...
# =============================================================================
# Electricity + utilities detection
# =============================================================================
...
# =============================================================================
# First-layer localization helpers (kept from v15)
# =============================================================================
...
def build_one_scenario(
    ...
    *,
    ...
    # 1) Gate A clone (uncertainty-safe), drop routing exchange by ref product
    ...
    psa = upsert_fg_activity(
        ...
        comment="PSA proxy cloned. Uncertainty-safe clone where present.",
        logger=logger,
        apply=apply,
    )
    ...
    # 6) NaOH proxy + electrolyte (NaOH proxy cloned; electrolyte authored deterministic)
    ...
    # 7) Hydrolysis C3C4 (authored deterministic; gate basis)
    ...
    # Stage D receiving markets (proxy clones + optional first-layer localization)
    ...
    # Stage D offsets (credit-only deterministic)
    ...
    *,
    ...
    if apply:
        if project_name not in ALLOWED_PROJECTS:
            raise RuntimeError(f"[safety] Refusing to APPLY in unexpected project: {project_name}. Allowed: {sorted(ALLOWED_PROJECTS)}")
        if fg_db_name not in ALLOWED_FG_DBS:
            raise RuntimeError(f"[safety] Refusing to APPLY in unexpected FG DB: {fg_db_name}. Allowed: {sorted(ALLOWED_FG_DBS)}")
    ...
    if fg_db_name not in bw.databases:
        if apply:
            ...
            raise RuntimeError(f"Foreground DB '{fg_db_name}' not found. In dry-run mode, the DB must already exist.")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Hydrolysis prospective BG-uncertainty builder (dry-run by default).")

    p.add_argument("--layer", choices=["bgonly", "fgonly", "joint"], default=os.environ.get("BW_UNC_LAYER", DEFAULT_LAYER))
    p.add_argument("--project", default=os.environ.get("BW_PROJECT", ""))  # if blank, derived from layer
    p.add_argument("--fg-db", default=os.environ.get("BW_FG_DB", ""))      # if blank, derived from layer

    p.add_argument("--scenario-ids", nargs="+", default=[s["id"] for s in DEFAULT_SCENARIOS])
    p.add_argument("--bg-map", action="append", default=None, help="Map scenario to BG db: SSP2M_2050=db_name (repeatable)")
...
def main() -> None:
    ...
    logger.info(f"[cfg] apply={apply} dry_run={bool(args.dry_run)}")

    fg_db = set_project_and_get_fg(project, fg_db_name, logger, apply=apply)
    ...
    # Scenario selection + bg map override
    want_sids = [s.strip() for s in (args.scenario_ids or []) if s.strip()]
    defaults_map = {s["id"]: s["bg_db"] for s in DEFAULT_SCENARIOS}
    user_map = parse_bg_map(args.bg_map)

    scenario_plan: List[Dict[str, str]] = []
    for sid in want_sids:
        bg_db = user_map.get(sid, defaults_map.get(sid))
        if not bg_db:
            raise KeyError(f"No BG DB mapping found for scenario '{sid}'. Provide --bg-map {sid}=<db_name>.")
        scenario_plan.append({"id": sid, "bg_db": bg_db})

    if args.dry_run and not apply:
        logger.info("=== DRY RUN MODE: auditing BG uncertainty + existing FG targets (no writes) ===")
        for s in scenario_plan:
    ...
    logger.info("=== APPLY MODE: rebuilding Hydrolysis with BG uncertainty carryover ===")
    ...
    if ureport.missing_rows:
        logger.info(
            ...
            "This is treated as a dataset limitation (no uncertainty invented)."
        )

    logger.info("[done] APPLY rebuild complete (hydrolysis; layer=%s; %d SSPs).", layer, len(scenario_plan))
