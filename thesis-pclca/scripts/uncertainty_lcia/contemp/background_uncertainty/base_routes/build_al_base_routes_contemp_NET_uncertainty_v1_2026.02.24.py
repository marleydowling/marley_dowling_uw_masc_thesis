# -*- coding: utf-8 -*-
"""
build_al_base_routes_contemp_NET_uncertainty_v1_26.02.24.py
...
- Landfill treatment proxy
- Degreasing proxy
- Post-consumer refiner proxy
- Avoided ingot / avoided extrusion proxies (legacy)
- Canonical Stage D ingot credit proxy:
    AL_credit_primary_ingot_IAI_NA_QC_elec
  including optional MARKET upstream provider cloning (and QC electricity swapping)

It does NOT invent uncertainty for deterministic/missing source exchanges; instead it logs and
writes a "missing uncertainty" report as a dataset limitation.
...
Dry run audit (no writes):
  python build_al_base_routes_contemp_NET_uncertainty_v1_26.02.24.py --dry-run

Apply rebuild (writes to uncertainty FG DB):
  python build_al_base_routes_contemp_NET_uncertainty_v1_26.02.24.py --apply

Optional post-audit (after apply):
  python build_al_base_routes_contemp_NET_uncertainty_v1_26.02.24.py --apply --post-audit

Recycle credit modes (same as v15):
...
# =============================================================================
# Constants (reuse degreasing kg -> m2) — identical to v15
# =============================================================================
...
# =============================================================================
# Electricity outputs (robust aliases) — identical to v15
# =============================================================================
...
# =============================================================================
# Template candidates (BG) — identical to v15
# =============================================================================
...
# =============================================================================
# Target FG codes (stable) — identical to v15
# =============================================================================
...
def load_registry(path: Path, logger: logging.Logger) -> Dict[str, Any]:
    ...
    logger.info(f"[registry] USE=True path={path}")
    return {"version": "al_base_routes_contemp_uncertainty_v1", "records": {}}
...
# =============================================================================
# BG indexing + template selection (kept aligned with v15)
# =============================================================================
...
# =============================================================================
# Electricity / provider swap (kept aligned with v15)
# =============================================================================
...
# =============================================================================
# Uncertainty copying + reporting (ported from MS-FSC uncertainty script)
# =============================================================================
...
def rescale_exchange_uncertainty_inplace(
    ...
    *,
    ...
    """
    # Mirror copy function semantics by cloning into a temp dict-like approach:
    ...
    if ut_i == UTYPE_LOGNORMAL and abs(float(amount_new)) < 1e-30:
        _record_missing("lognormal_zero_reset")
        # leave uncertainty fields as-is (rare edge); caller should avoid zero
        return
    ...
    *,
    ...
    if credit_kind == "market" and CLONE_MARKET_PROVIDERS_UPSTREAM:
        ...
        if apply:
            ...
            # Identify direct provider inputs (positive technosphere), excluding electricity
            ...
            if len(providers) > MAX_MARKET_PROVIDERS_TO_CLONE:
                logger.warning("[canonical-credit] Market has %d providers; cloning first %d only.",
                               len(providers), MAX_MARKET_PROVIDERS_TO_CLONE)

            rewired = 0
            for exc, prov in providers[:MAX_MARKET_PROVIDERS_TO_CLONE]:
        ...
        # Dry-run: just inspect source coverage; no writes
        clone_exchanges_uncertainty_safe(
            src, src, scale=1.0,
            logger=logger, ustats=ustats, ureport=ureport,
...
# =============================================================================
# Recycling credit handling helpers (same detection logic as v15)
# =============================================================================
    ...
    *,
    ...
    for exc in list(refiner.exchanges()):
        ...
        if _looks_like_aluminium_product_provider(prov):
            ...
            exc["amount"] = amt_new
            # rescale uncertainty (in-place) consistent with amount scaling
...
def post_build_audit(
    ...
    *,
    ...
    logger.info("[post] refiner_src uncertain=%d/%d | refiner_fg uncertain=%d/%d",
                src_ref["uncertain"], src_ref["total"], tgt_ref["uncertain"], tgt_ref["total"])


# =============================================================================
# Main build (apply) — v15 logic preserved, uncertainty-safe cloning added
# =============================================================================
...
def main():
    ...
    # Switch to the target project BEFORE any bd.databases inspection (e.g., detect_bg_db)
    ...
    if args.dry_run and not apply:
        ...
        # Also inspect canonical credit source uncertainty coverage
    ...
    credit_src_key, credit_kind = resolve_canonical_credit_source(logger, bg_db, idx_name)
    credit_proxy = build_canonical_credit_proxy(
        logger,
        fg_db=fg_db,
        bg_db=bg_db,
    ...
    # 6) Recycling credit handling
    sd_rec = None
    up_ref_burdens = None

    if args.recycle_credit_mode == "external_stageD":
        # clone burdens-only from current refiner (FG->FG) preserving uncertainty, then strip credits
        up_ref_burdens = upsert_fg_activity(
            fg_db,
            CODES["UP_refiner_postcons_no_credit"],
            name=f"{up_refiner.get('name','Refiner')} [NO embedded credit; external_stageD]",
        ...
        # Stage D credit node (authored; deterministic exchange amount; provider itself may carry uncertainty)
    ...
    # 7) Reuse Stage D credits (explicit) — canonical ingot + extrusion (authored deterministic)
    ...
    # 8) Wrappers (authored deterministic)
    up_ref_for_c3c4 = up_ref_burdens if (args.recycle_credit_mode == "external_stageD" and up_ref_burdens is not None) else up_refiner
    ...
    rw_rec = upsert_fg_activity(
        ...
        name="Route wrapper: Aluminium recycling (C3–C4), CA — refiner",
        location="CA", unit="kilogram", ref_product="route wrapper service",
        logger=logger, apply=True,
    )
    ...
    rw_rec_net = upsert_fg_activity(
        ...
        name="Route wrapper NET: Aluminium recycling post-consumer (C3–C4) + Stage D where applicable",
        location="CA", unit="kilogram", ref_product="route wrapper service",
        logger=logger, apply=True,
    )
    ...
    logger.info("[done] APPLY complete. Contemporary base routes rebuilt (uncertainty-safe). Canonical Stage D ingot provider aligned to '%s'.",
                CANONICAL_INGOT_CREDIT_CODE)
