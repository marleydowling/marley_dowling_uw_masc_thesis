# -*- coding: utf-8 -*-
"""
build_al_base_routes_contemp_NET_uncertainty_v2_26.02.24.py
...
2) **Backups before rebuild**
   - In --apply mode, backs up any existing target FG activities to:
       <code>__BAK__<timestamp>
   - Disable with --no-backup if desired.

3) Canonical market provider cloning selection improvement
...
def backup_activity_if_exists(
    ...
    *,
    ...
    """
    Create a full backup clone of an existing activity (including exchanges) as:
      <code>__BAK__<ts>
    Returns the backup activity if created.
    """
    if not enabled:
        return None
    ...
    try:
        # avoid duplicate backup code collisions
    ...
    # Copy core metadata (be conservative)
    ...
    # Preserve any additional keys that are safe-ish
    ...
    for exc in src.exchanges():
        ...
        # Copy remaining exchange fields
    ...
    *,
    ...
    if credit_kind == "market" and CLONE_MARKET_PROVIDERS_UPSTREAM:
        ...
        if apply:
            ...
            # v2: choose most material providers by amount (desc)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--no-backup", action="store_true", help="Disable pre-rebuild backups of existing target activities.")
...
def main():
    ...
    # -------------------------------------------------------------------------
    # 6) v2 Decomposition-correct recycling structure (always build both artifacts)
    # -------------------------------------------------------------------------

    # 6a) Burdens-only clone from up_refiner (FG->FG) preserving uncertainty, then strip embedded credits
    ...
    # 6b) Explicit Stage D node using inferred yield (exists even if rewire_embedded, for audit/decomposition)
    ...
    # 6c) Mode-specific handling for the BASE refiner (embedded credit) if chosen
    if args.recycle_credit_mode == "rewire_embedded":
        ...
        # leave embedded credits in up_refiner untouched; decomposition uses up_ref_burdens + sd_rec
        pass
    ...
    # 8) Wrappers (authored deterministic) — v2 FIX: C3C4 recycling wrapper uses burdens-only
    ...
    # v2 FIX: NET composition depends on credit mode
    if args.recycle_credit_mode == "rewire_embedded":
        # Embedded credit is inside up_refiner
        rw_rec_net.new_exchange(input=up_refiner.key, amount=1.0, type="technosphere", unit="kilogram").save()
    else:
        # External Stage D decomposition: burdens wrapper + explicit Stage D node
