# -*- coding: utf-8 -*-
r"""
build_al_base_routes_contemp_NET_uncertainty_v3_26.02.25.py
...
Key v3 fixes vs v2
------------------
1) **Backups are now OPT-IN (disabled by default)**
   - Use --backup to create __BAK__ clones before rebuild.
   - Default behavior keeps the uncertainty sandbox clean (no BAK clutter).

2) **Production-safe backups (if --backup is used)**
   - Backup clones NEVER copy production exchanges verbatim (prevents prod_not_self).
   - Backup clones ALWAYS regenerate a single self-production exchange.

Everything else remains aligned with v2:
- Decomposition-correct recycling structure across credit modes
- Canonical credit proxy creation + optional market provider cloning
- Uncertainty metadata copying / scaling rules
- Safe-by-default apply gating (requires *_uncertainty_analysis)

Execution
---------
Dry run audit (no writes):
  python build_al_base_routes_contemp_NET_uncertainty_v3_26.02.25.py --dry-run

Apply rebuild (writes to uncertainty FG DB):
  python build_al_base_routes_contemp_NET_uncertainty_v3_26.02.25.py --apply

Apply rebuild + backups (opt-in):
  python build_al_base_routes_contemp_NET_uncertainty_v3_26.02.25.py --apply --backup

Optional post-audit (after apply):
  python build_al_base_routes_contemp_NET_uncertainty_v3_26.02.25.py --apply --post-audit
...
def load_registry(path: Path, logger: logging.Logger) -> Dict[str, Any]:
    ...
    logger.info(f"[registry] USE=True path={path}")
    return {"version": "al_base_routes_contemp_uncertainty_v3", "records": {}}
...
# =============================================================================
# Uncertainty copying + reporting (ported / kept consistent with v1/v2)
# =============================================================================
...
def backup_activity_if_exists(
    ...
    *,
    ...
    """
    Create a backup clone of an existing activity (including exchanges) as:
      <code>__BAK__<ts>

    PRODUCTION-SAFE POLICY:
    - Never copy production exchanges verbatim (they would point to the source activity key).
    - Always regenerate a single self-production exchange for the backup activity.
    """
    if not enabled:
        return None
    ...
    bak_code = f"{code}__BAK__{ts}"
    try:
        _ = fg_db.get(bak_code)
        logger.warning("[backup] Backup code already exists; skipping: %s", bak_code)
        return None
    except Exception:
        pass

    bak = fg_db.new_activity(bak_code)

    bak["name"] = f"{src.get('name','(activity)')} [BAK {ts}]"
    bak["location"] = src.get("location", "")
    bak["unit"] = src.get("unit", "")
    bak["reference product"] = src.get("reference product", "")
    bak["type"] = src.get("type", "process")

    for k, v in dict(src).items():
        if k in ("database", "code", "id"):
            continue
        if k in ("name", "location", "unit", "reference product", "type"):
            continue
        try:
            bak[k] = v
        except Exception:
    ...
    # Copy exchanges EXCEPT production
    for exc in src.exchanges():
        if exc.get("type") == "production":
            continue

        inp_key = exc.input.key
        new_exc = bak.new_exchange(input=inp_key, amount=float(exc.get("amount") or 0.0), type=exc.get("type"))
        for ek, ev in dict(exc).items():
            if ek in ("input", "output", "amount", "type"):
                continue
            try:
                new_exc[ek] = ev
            except Exception:
    ...
    # Force exactly one self-production exchange (prevents prod_not_self)
    ensure_single_production(bak, bak.get("unit", "") or src.get("unit", ""))

    logger.info("[backup] (%s,%s) -> (%s,%s)", src.key[0], code, bak.key[0], bak_code)
    return bak
...
# =============================================================================
# Main build (apply) — v15 logic preserved, v2 decomposition fix kept
# =============================================================================
...
def parse_args() -> argparse.Namespace:
    ...
    # v3: backups are opt-in (OFF by default)
    p.add_argument("--backup", action="store_true", help="Create __BAK__ clones before rebuild (OFF by default).")
...
def main():
    ...
    # v3: backups are opt-in
    backup_enabled = bool(args.backup)
    if backup_enabled:
        logger.info("[apply] Backing up existing target FG activities (opt-in --backup)...")
        target_codes = [CANONICAL_INGOT_CREDIT_CODE] + sorted(set(CODES.values()))
        for code in target_codes:
            backup_activity_if_exists(fg_db, code, ts, logger, enabled=True)
    else:
        logger.info("[apply] Backups disabled (default). No __BAK__ activities will be created.")

    # 1) Canonical Stage D ingot credit proxy first
    credit_src_key, credit_kind = resolve_canonical_credit_source(logger, bg_db, idx_name)
    ...
    # 2) Landfill clone + swaps (CA elec mode)
    ...
    # 3) Degreasing clone + swaps (CA elec mode)
    ...
    # 4) Refiner clone + swaps (CA elec mode) + probe negative technosphere
    ...
    # 5) Legacy avoided proxies (kept for backwards compatibility / optional use)
    ...
    # 6) Decomposition-correct recycling structure
    ...
    # 7) Reuse Stage D credits (explicit)
    ...
    # 8) Wrappers — C3C4 recycling wrapper uses burdens-only
    ...
    logger.info(
        "[done] APPLY complete. Contemporary base routes rebuilt (uncertainty-safe, v3). "
        "Canonical Stage D ingot provider aligned to '%s'. Recycling decomposition fixed. Backups=%s.",
        CANONICAL_INGOT_CREDIT_CODE,
        "ON" if backup_enabled else "OFF",
    )
